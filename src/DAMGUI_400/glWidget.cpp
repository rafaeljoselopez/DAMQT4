...
//    connections << connect(gdock,SIGNAL(resizedialog(QSize)),QDLwindow,SLOT(renewwidth(QSize)));
    connect(gdock,&MyDockWidget::resizedialog,
            QDLwindow,&moleculeDialog::renewwidth);
...
//    connections << connect(mainWin,SIGNAL(hide_viewer()),this,SIGNAL(hideviewer()));
    connect(mainWin,&MainWindow3DViewer::hide_viewer,
            this,&glWidget::hideviewer);
...
void glWidget::createWindowDialog(MyDockWidget *gdock){
    gdock->resize(QSize(500, this->height()));
...
//    connections << connect(gdock,SIGNAL(resizedialog(QSize)),QDLwindow,SLOT(renewwidth(QSize)));
    ...
    connect(QDLwindow,&moleculeDialog::closedialog,
            this,&glWidget::close);
    connect(mainWin,&MainWindow3DViewer::hide_viewer,
            this,&glWidget::hideviewer);
    connect(gdock,&MyDockWidget::resizedialog,
...
//    connections << connect(BTNaddmolecule, SIGNAL(clicked()), this, SLOT(addmoleculeTowindow()));
    connect(BTNaddmolecule,&QPushButton::clicked,
            this,&glWidget::addmoleculeTowindow);
...
//    connections << connect(BTNmeasures, SIGNAL(clicked()), this, SLOT(BTNmeasures_clicked()));
    connect(BTNmeasures,&QPushButton::clicked,
            this,&glWidget::BTNmeasures_clicked);
...
//        connections << connect(CHKmol, SIGNAL(stateChanged(int)), molecules->at(i), SLOT(toggleactive()));
        connect(CHKmol,&QCheckBox::stateChanged,
                molecules->at(i),&molecule::toggleactive);
...
//        connections << connect(BTNmol, SIGNAL(clicked()), molecules->at(i), SLOT(editmolecule()));
        connect(BTNmol,&QPushButton::clicked,
                molecules->at(i),&molecule::editmolecule);
...
//        connections << connect(BTNdeletemol, SIGNAL(clicked()), deletesignalMapper, SLOT(map()), Qt::UniqueConnection);
        connect(BTNdeletemol,&QPushButton::clicked,
                this,[this, i]() { deletemolecule(i); });
...
void glWidget::create_rotation_menu()
{
    ...
    connect(BTNrotation,&QPushButton::clicked,
            this,&glWidget::BTNrotation_clicked);

    connect(QDLwindow->rotationsdialog,&rotations::rotation_changed,
            this,&glWidget::rotation_changed);

    connect(QDLwindow->rotationsdialog,&rotations::chkrotatechanged,
            this,&glWidget::CHKrotate_changed);

    connect(QDLwindow->rotationsdialog,&rotations::animationclicked,
            this,&glWidget::toggleanimation);

    connect(timer,&QTimer::timeout,
            this,&glWidget::animaterotation);

    connect(QDLwindow->rotationsdialog,&rotations::SLDspeed_changed,
            this,&glWidget::resetinterval);

    connect(QDLwindow->rotationsdialog->recorddialog,&recorder::startrecording,
            this,&glWidget::startrecording);

    connect(QDLwindow->rotationsdialog->recorddialog,&recorder::stoprecording,
            this,&glWidget::stoprecording);

    connect(QDLwindow->rotationsdialog->recorddialog,&recorder::recordfilenamechanged,
            this,&glWidget::set_recordfilename);
}
...
//    connections << connect(QDLwindow->translationsdialog,SIGNAL(stepwheel_changed(float)),window,SLOT(setstepwheel(float)));
    ...
    connect(BTNtranslation,&QPushButton::clicked,
            this,&glWidget::BTNtranslation_clicked);

    connect(QDLwindow->translationsdialog,&translations::setworldtranslation,
            window,&glWindow::setworld_translation);

    connect(QDLwindow->translationsdialog,&translations::stepwheel_changed,
            window,&glWindow::setstepwheel);
}
...
void glWidget::create_axes_menu()
{
    ...
    connect(BTNaddaxes,&QPushButton::clicked,
            this,&glWidget::BTNaddaxes_clicked);

    connect(QDLwindow->axesdialog,&axes::axesvisible,
            window,&glWindow::setaxesvisible);

    connect(QDLwindow->axesdialog,&axes::axeslabelsvisible,
            window,&glWindow::setaxeslabelsvisible);

    connect(QDLwindow->axesdialog,&axes::axesfont,
            window,&glWindow::setfontlabaxeslabels);

    connect(QDLwindow->axesdialog,&axes::axesarrowsize,
            window,&glWindow::setaxesarrowsize);

    connect(QDLwindow->axesdialog,&axes::axesarrowwidth,
            window,&glWindow::setaxesarrowwidth);

    connect(QDLwindow->axesdialog,&axes::axeslength,
            window,&glWindow::setaxeslength);

    connect(QDLwindow->axesdialog,&axes::axesthickness,
            window,&glWindow::setaxesthickness);

    connect(QDLwindow->axesdialog,&axes::xaxiscolor,
            window,&glWindow::setXaxis_color);

    connect(QDLwindow->axesdialog,&axes::yaxiscolor,
            window,&glWindow::setYaxis_color);

    connect(QDLwindow->axesdialog,&axes::zaxiscolor,
            window,&glWindow::setZaxis_color);
}
...
//    connections << connect(QDLwindow->scrshotdialog, SIGNAL(take_picture()), this, SLOT(capture()));
    ...
    connect(BTNcapture,&QPushButton::clicked,
            this,&glWidget::BTNcapture_clicked);

    connect(QDLwindow->scrshotdialog,&screenshot::scaledef_changed,
            this,&glWidget::adjustQDLSize);

    connect(QDLwindow->scrshotdialog,&screenshot::take_picture,
            this,&glWidget::capture);
}
...
//    connections << connect(QDLwindow->lightsdialog, SIGNAL(updateandmoveToTop()), this, SLOT(emitmovetotop()));
    ...
    connect(BTNlights,&QPushButton::clicked,
            this,&glWidget::BTNlights_clicked);

    connect(QDLwindow->lightsdialog,&lights::ambientcolorchanged,
            window,&glWindow::setAmbientColor);

    connect(QDLwindow->lightsdialog,&lights::bkgcolorchanged,
            window,&glWindow::setBkgColor);

    connect(QDLwindow->lightsdialog,&lights::lightcolorchanged,
            window,&glWindow::setLightColor);

    connect(QDLwindow->lightsdialog,&lights::specularcolorchanged,
            window,&glWindow::setSpecularColor);

    connect(QDLwindow->lightsdialog,&lights::lightschanged,
            window,&glWindow::setlightsposition);

    connect(QDLwindow->lightsdialog,&lights::lightpowerchanged,
            window,&glWindow::setLightPower);

    connect(QDLwindow->lightsdialog,&lights::attenuationchanged,
            window,&glWindow::setlinearattenuation);

    connect(QDLwindow->lightsdialog,&lights::specularindexchanged,
            window,&glWindow::setSpecularIndex);

    connect(QDLwindow->lightsdialog,&lights::updateandmoveToTop,
            this,&glWidget::emitmovetotop);
...
//    connections << connect(QDLwindow->ballsandcylsdialog,SIGNAL(ballcylchanged()),this,SLOT(ballcyl_changed()));
    ...
    connect(BTNballcyl,&QPushButton::clicked,
            this,&glWidget::BTNballcyl_clicked);

    connect(QDLwindow->ballsandcylsdialog,&ballscyls::scaleradiichanged,
            this,&glWidget::CHKscaleradii_changed);

    connect(QDLwindow->ballsandcylsdialog,&ballscyls::ballcylchanged,
            this,&glWidget::ballcyl_changed);
}
...
//    connections << connect(QDLwindow->viewportdialog,SIGNAL(viewportchanged()),this,SLOT(viewport_changed()));
    ...
    connect(BTNviewport,&QPushButton::clicked,
            this,&glWidget::BTNviewport_clicked);

    connect(QDLwindow->viewportdialog,&viewport::viewportchanged,
            this,&glWidget::viewport_changed);
...
//    connections << connect(QDLwindow->mespimizerdialog, SIGNAL(obabelindex_changed(int)), this, SLOT(obabelindex_changed(int)));
    ...
    connect(BTNoptimizeCluster,&QPushButton::clicked,
            this,&glWidget::BTNoptimizeCluster_clicked);

    connect(QDLwindow->mespimizerdialog,&mespimizer::movetotop,
            this,&glWidget::emitmovetotop);

    connect(QDLwindow->mespimizerdialog,&mespimizer::adjustQDL,
            this,&glWidget::adjustQDLSize);

    connect(QDLwindow->mespimizerdialog,&mespimizer::clusterfile_changed,
            this,&glWidget::clusterfile_changed);

    connect(QDLwindow->mespimizerdialog,&mespimizer::framesfile_changed,
            this,&glWidget::framesfile_changed);

    connect(QDLwindow->mespimizerdialog,&mespimizer::deletecluster,
            this,&glWidget::deletecluster);

    connect(QDLwindow->mespimizerdialog,&mespimizer::exec_mespimizer,
            this,&glWidget::exec_mespimizer);

    connect(QDLwindow->mespimizerdialog,&mespimizer::optimizecanvas_changed,
            this,&glWidget::optimizecanvas_changed);

    connect(QDLwindow->mespimizerdialog,&mespimizer::qmrun,
            this,&glWidget::qmrun);

    connect(QDLwindow->mespimizerdialog,&mespimizer::replay,
            this,&glWidget::replay_mespimization);

    connect(QDLwindow->mespimizerdialog,&mespimizer::reset,
            this,&glWidget::reset_mespimization);

    connect(QDLwindow->mespimizerdialog,&mespimizer::recordfilenamechanged,
            this,&glWidget::set_recordfilename);

    connect(QDLwindow->mespimizerdialog,&mespimizer::speed_changed,
            this,&glWidget::speed_changed);

    connect(QDLwindow->mespimizerdialog,&mespimizer::interpol_changed,
            this,&glWidget::interpol_changed);

    connect(QDLwindow->mespimizerdialog,&mespimizer::recordoptim_changed,
            this,&glWidget::recordoptim_changed);

    connect(QDLwindow->mespimizerdialog,&mespimizer::template_changed,
            this,&glWidget::template_changed);

    connect(QDLwindow->mespimizerdialog,&mespimizer::optimizeselect_changed,
            this,&glWidget::optimizeselect_changed);

    connect(QDLwindow->mespimizerdialog,&mespimizer::displayepic_changed,
            this,&glWidget::setdisplayEPIC);

    connect(QDLwindow->mespimizerdialog,&mespimizer::energyprecision_changed,
            this,&glWidget::setenergyprecision);

    connect(QDLwindow->mespimizerdialog,&mespimizer::font_clicked,
            this,&glWidget::setenergyfont);

    connect(QDLwindow->mespimizerdialog,&mespimizer::fontcolor_clicked,
            this,&glWidget::setenergycolor);

    connect(QDLwindow->mespimizerdialog,&mespimizer::hartree_units,
            this,&glWidget::sethartree);

    connect(QDLwindow->mespimizerdialog,&mespimizer::obabelcharges_changed,
            this,&glWidget::obabelcharges_changed);

    connect(QDLwindow->mespimizerdialog,&mespimizer::obabelindex_changed,
            this,&glWidget::obabelindex_changed);
...
//    connections << connect(BTNsaveGeometry,SIGNAL(clicked(bool)),this,SLOT(BTNsaveGeometry_clicked()));
    ...
    connect(BTNsaveGeometry,&QPushButton::clicked,
            this,&glWidget::BTNsaveGeometry_clicked);
}
...
void glWidget::create_save_retrieve_menu()
{
    ...
    BTNsettings = new QPushButton(tr("Save/retrieve settings"));

    connect(BTNsettings,&QPushButton::clicked,
            this,&glWidget::BTNsettings_clicked);

    BTNsaveSettings = new QPushButton(tr("Save"));

    connect(BTNsaveSettings,&QPushButton::clicked,
            this,&glWidget::BTNsaveSettings_clicked);

    BTNretrieveSettings = new QPushButton(tr("Retrieve"));

    connect(BTNretrieveSettings,&QPushButton::clicked,
            this,&glWidget::BTNretrieveSettings_clicked);
    ...
    connect(CHKsettingsmolecs,&QCheckBox::stateChanged,
            this,&glWidget::CHKsettingsmolecs_changed);
    ...
    connect(CHKsettingsgrids,&QCheckBox::stateChanged,
            this,&glWidget::CHKsettingsgrids_changed);
...
//        connections << connect(window,SIGNAL(update_worldtranslation(QVector3D)),this,SLOT(settranslation(QVector3D)));
        ...
        connect(window, &glWindow::endrecording,
                this,&glWidget::stoprecording);

        connect(window,&glWindow::endmakingmovie,
                this,&glWidget::endmakingmovie);

        connect(window,&glWindow::emitcheckactivate,
                this,&glWidget::checkactivate);

        connect(window,&glWindow::update_worldrotation,
                this,&glWidget::setrotation);

        connect(window,&glWindow::update_worldtranslation,
                this,&glWidget::settranslation);
    }
...
void glWidget::initQDLpointers(){
    ...
    BTNlights = nullptr;
    BTNmeasures = nullptr;
    BTNoptimizeCluster = nullptr;
    BTNretrieveSettings = nullptr;
    BTNrotation = nullptr;
    BTNsaveGeometry = nullptr;
    BTNsaveSettings = nullptr;
    BTNsettings = nullptr;
    BTNviewport = nullptr;

    CHKsettingsballs = nullptr;
    CHKsettingsbkg = nullptr;
    CHKsettingsgrids = nullptr;
    CHKsettingsisosurf = nullptr;
    CHKsettingslights = nullptr;
    CHKsettingsmolecs = nullptr;
    CHKsettingspos = nullptr;
    CHKsettingsrot = nullptr;
    CHKsettingssurf = nullptr;
    CHKsettingsviewport = nullptr;

    FRMballcyl = nullptr;
    FRMcapture = nullptr;
    FRMlights = nullptr;
    FRMmeasures = nullptr;
    FRMoptimizeCluster = nullptr;
    FRMrotation = nullptr;
    FRMsaveGeometry = nullptr;
    FRMsettings = nullptr;
    FRMtranslation = nullptr;
    FRMviewport = nullptr;

    layoutmolecules = nullptr;

    SPBbondthres = nullptr;
}
...
void glWidget::create_measures()
{
    ...
    connect(msrs,&measures::activate_measure,
            window,&glWindow::setmeasures);
    connect(msrs,&measures::angles_precision,
            window,&glWindow::setanglesprecision);
    connect(msrs,&measures::angles_type,
            window,&glWindow::setanglestype);
    connect(msrs,&measures::angles_width,
            window,&glWindow::setangleswidth);
    connect(msrs,&measures::angstrom_changed,
            window,&glWindow::setangstrom);
    connect(msrs,&measures::arc_radius,
            window,&glWindow::setarcradius);

    connect(msrs,&measures::color_angles,
            window,&glWindow::setanglescolor);
    connect(msrs,&measures::color_dihedrals,
            window,&glWindow::setdihedralscolor);
    connect(msrs,&measures::color_distances,
            window,&glWindow::setdistancescolor);

    connect(msrs,&measures::dihedrals_precision,
            window,&glWindow::setdihedralsprecision);
    connect(msrs,&measures::dist_precision,
            window,&glWindow::setdistprecision);
    connect(msrs,&measures::dist_vshift,
            window,&glWindow::setdistvshift);
    connect(msrs,&measures::dist_transpbkg,
            window,&glWindow::setdisttranspbkg);

    connect(msrs,&measures::draw_arcs,
            window,&glWindow::setdrawarcs);
    connect(msrs,&measures::draw_lines,
            window,&glWindow::setdrawlines);

    connect(msrs,&measures::emit_update_angles,
            window,&glWindow::emit_update_angles);
    connect(msrs,&measures::emit_update_dihedrals,
            window,&glWindow::emit_update_dihedrals);
    connect(msrs,&measures::emit_update_distances,
            window,&glWindow::emit_update_distances);

    connect(msrs,&measures::font_angles,
            window,&glWindow::setanglesfont);
    connect(msrs,&measures::font_dihedrals,
            window,&glWindow::setdihedralsfont);
    connect(msrs,&measures::font_distances,
            window,&glWindow::setdistancesfont);

    connect(msrs,&measures::lines_type,
            window,&glWindow::setlinestype);
    connect(msrs,&measures::lines_width,
            window,&glWindow::setlineswidth);

    connect(msrs,&measures::measure_angles,
            window,&glWindow::setangles);
    connect(msrs,&measures::measure_dihedrals,
            window,&glWindow::setdihedrals);
    connect(msrs,&measures::measure_distances,
            window,&glWindow::setdistances);

    connect(msrs,&measures::reset_angles,
            window,&glWindow::resetangles);
    connect(msrs,&measures::reset_dihedrals,
            window,&glWindow::resetdihedrals);
    connect(msrs,&measures::reset_distances,
            window,&glWindow::resetdistances);

    connect(msrs,&measures::show_angles,
            window,&glWindow::setshowangles);
    connect(msrs,&measures::show_dihedrals,
            window,&glWindow::setshowdihedrals);
    connect(msrs,&measures::show_distances,
            window,&glWindow::setshowdistances);

    connect(msrs,&measures::dihedralplanes_color,
            window,&glWindow::setdihedralplanescolor);

    connect(msrs,&measures::QDLmeasures_rejected,
            this,&glWidget::QDLmeasures_rejected);
    connect(msrs,&measures::get_systemnames,
            this,&glWidget::send_systemnames);

    connect(window,&glWindow::select_angle,
            msrs,&measures::LBLangles_add);
    connect(window,&glWindow::select_dihedral,
            msrs,&measures::LBLdihedrals_add);
    connect(window,&glWindow::select_distance,
            msrs,&measures::LBLdistances_add);

    connect(window,&glWindow::update_angles,
            msrs,&measures::update_angles);
    connect(window,&glWindow::update_dihedrals,
            msrs,&measures::update_dihedrals);
    connect(window,&glWindow::update_distances,
            msrs,&measures::update_distances);

//    connect(window,&glWindow::remove_distance,
//            msrs,&measures::distance_remove);

    connect(window,&glWindow::resetlastselectangles,
            msrs,&measures::resetlastselectangles);
    connect(window,&glWindow::resetlastselectdihedrals,
            msrs,&measures::resetlastselectdihedrals);
    connect(window,&glWindow::resetlastselectdist,
            msrs,&measures::resetlastselectdist);
    connect(window,&glWindow::resetbohr,
            msrs,&measures::resetRBTbohr);
}
...
void glWidget::ballcyl_changed()
{
    auto *balls = QDLwindow->ballsandcylsdialog;

    ballradius = balls->getballrad();
    cylradius  = balls->getcylrad();

    const double bondThreshold =
        balls->getbondthres() * ANGSTROM_TO_BOHR;

    disthressq = bondThreshold * bondThreshold;

    for (int i = 0; i < molecules->count(); i++) {
        auto *mol = molecules->at(i);

        mol->setballradius(ballradius);
        mol->setcylradius(cylradius);
        mol->setdisthressq(disthressq);
        mol->makeStructureBondsandSticks();
    }
    updatedisplay();
}
...
//        connections << connect(molecules->last(), SIGNAL(animate(bool)), this, SLOT(animate(bool)));
        ...
        connections << connect(molecules->last(),&molecule::updatedisplay,
                               this,&glWidget::updatedisplay);
        connections << connect(molecules->last(),&molecule::updateGL,
                               this,&glWidget::updateGL);
        connections << connect(molecules->last(),&molecule::animate,
                               this,&glWidget::animate);
        return true;
    }
}
...
//    connections << connect(molecules->last(), SIGNAL(animate(bool)), this, SLOT(animate(bool)));
    ...
    connect(molecules->last(),&molecule::updatedisplay,
            this,&glWidget::updatedisplay);

    connect(molecules->last(),&molecule::updateGL,
            this,&glWidget::updateGL);

    connect(molecules->last(),&molecule::animate,
            this,&glWidget::animate);
...
void glWidget::exec_mespimizer()
{
    ...
    const QString stdinput = mespimizerpath + "/mespimizer.inp";

    const QString stdoutput = mespimizerpath + "/mespimizer.out";

    if (!strprocess)
        strprocess = new QString();

    if (!arguments)
        arguments = new QStringList();

    *strprocess = QStringLiteral("DAMMESPIMIZER.exe");

    const QString execName =
        get_execName(*strprocess, QStringLiteral("TDAM320"));
    ...
    /*
     * QProcess already redirects the standard input and output files,
     * so shell redirection operators (< and >) are not required.
     */
    // *arguments << " < " << stdinput << " > " << stdoutput;
    ...
    if (!myProcess) {
        ...
        connect(myProcess,&QProcess::started,
                this,&glWidget::processStart);

        connect(myProcess,QOverload<QProcess::ProcessError>::of(&QProcess::errorOccurred),
                this,&glWidget::processError);

        connect(myProcess,QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
                this,&glWidget::processOutput);
    }
    ...
    myProcess->setStandardErrorFile(
        stdoutput,
        QIODevice::Append);
    ...
    if (!watcher) {
        ...
        connect(watcher,&QFileSystemWatcher::fileChanged,
                this,&glWidget::displayClusterGeometry);
    }

    /*
     * Stop watching files from a previous execution. This is important
     * if the mespimizer directory has changed.
     */
    if (!watcher->files().isEmpty())
        watcher->removePaths(watcher->files());

    /*
     * QFileSystemWatcher can only watch an existing file. If
     * mespimizer.out does not exist yet, create an empty file before
     * adding it to the watcher.
     */
    QFile outputFile(stdoutput);
    ...
    if (QFile::exists(stdoutput))
        watcher->addPath(stdoutput);
...
//void glWidget::BTNretrieveSettings_clicked()
//{
//    const QString fileName = selectSettingsFile();
...
//                .arg(fileName, fileIn.errorString())
//        );
...
//    CaptureFolder = QFileInfo(fileName).absolutePath();
//    const std::string iniFile = fileName.toStdString();

//    if (CHKsettingsmolecs->isChecked()
//            && !retrieveMoleculesSettings(iniFile)) {
//        emit moveToTop(windownumber);
//        return;
//    }

//    if (CHKsettingslights->isChecked())
//        retrieveLightsSettings(iniFile);

//    if (CHKsettingsbkg->isChecked())
//        retrieveBackgroundSettings(iniFile);

//    if (CHKsettingsballs->isChecked())
//        retrieveBallsAndCylindersSettings(iniFile);

//    if (CHKsettingsviewport->isChecked())
//        retrieveViewportSettings(iniFile);
...
void glWidget::BTNretrieveSettings_clicked()
{
    ...
    const QString fileName = fileDialog.getOpenFileName(
        this,
        tr("Open file ..."),
        molpath,
        tr("Settings saving files")
            + " *.3Dsettings (*.3Dsettings);;"
            + tr("All files") + " (*)"
    );
    ...
    const SettingsReader reader(fileName);
    const MoleculeSettingsLoader moleculeLoader(reader);

    if (CHKsettingsmolecs->isChecked()) {
        const int moleculeCount =
            reader.intValue("V3DMOLECULES","nummolecs");

        for (int i = 0; i < moleculeCount; i++) {
            const QString section =
                QString("V3DMOLECULE_%1").arg(i);

            const QString geometryFile =
                reader.value(section,"name");

            if (geometryFile.isEmpty())
                continue;

            const QFileInfo geometryInfo(geometryFile);

            set_ProjectFolder(geometryInfo.absolutePath());
            set_ProjectName(geometryInfo.completeBaseName());

            const QString suffix =
                QFileInfo(geometryFile).suffix().toLower();

            bool geometryLoaded = false;

            if (suffix == "xyz")
                geometryLoaded = readxyz(geometryFile);
            else if (suffix == "ggbs")
                geometryLoaded = readggbs(geometryFile);
            else {
                QMessageBox::warning(
                    this,
                    tr("DAMQT"),
                    tr("Unsupported molecular geometry format:\n%1")
                        .arg(geometryFile)
                );
            ...
            molecule *mol = molecules->last();

            mol->set_ProjectFolder(ProjectFolder);
            mol->set_ProjectName(ProjectName);

            moleculeLoader.applyMoleculeSettings(
                mol,
                section,
                CHKsettingspos->isChecked(),
                CHKsettingsrot->isChecked()
            );

            if (CHKsettingssurf->isChecked())
                moleculeLoader.loadSurfaces(mol,section);

            if (CHKsettingsgrids->isChecked())
                moleculeLoader.loadGrids(mol,section);
        }
    }
    ...
    if (CHKsettingslights->isChecked()) {
        ...
        if (reader.colorValue(
                "V3DLIGHTS","ambientcolor",color)) {
            ambientcolor = color;
            QDLwindow->lightsdialog->setambientcolor(color);
        ...
        if (reader.colorValue(
                "V3DLIGHTS","lightcolor",color)) {
            lightcolor = color;
            QDLwindow->lightsdialog->setlightcolor(color);

            if (window)
                window->setLightColor(color);
        }

        if (reader.colorValue(
                "V3DLIGHTS","specularcolor",color)) {
            specularcolor = color;
            QDLwindow->lightsdialog->setspecularcolor(color);

            if (window)
                window->setSpecularColor(color);
        }
    }
    ...
    if (CHKsettingsbkg->isChecked()) {
        ...
        if (reader.colorValue(
                "V3DBKG","bkgcolor",color)) {
            bkgcolor = color;
            QDLwindow->lightsdialog->setbkgcolor(color);

            if (window)
                window->setBkgColor(color);
        }
    }

    if (CHKsettingsballs->isChecked()) {
        const float savedBallRadius =
            reader.floatValue("V3DBALLS","ballradius");

        const float savedCylinderRadius =
            reader.floatValue("V3DBALLS","cylradius");

        if (savedBallRadius > 0.0f) {
            ballradius = savedBallRadius;
            QDLwindow->ballsandcylsdialog
                ->setballrad(ballradius);
        }

        if (savedCylinderRadius > 0.0f) {
            cylradius = savedCylinderRadius;
            QDLwindow->ballsandcylsdialog
                ->setcylrad(cylradius);
        }
    ...
    if (CHKsettingsviewport->isChecked()) {
        const float savedFov =
            reader.floatValue("V3DVIEWPORT","fov");

        const float savedNear =
            reader.floatValue("V3DVIEWPORT","zNear");

        const float savedFar =
            reader.floatValue("V3DVIEWPORT","zFar");
...
void glWidget::applyLightsSettings(const SettingsReader &reader)
{
    QColor color;

    if (reader.colorValue("V3DLIGHTS","ambientcolor",color)) {
        ambientcolor = color;
        QDLwindow->lightsdialog->setambientcolor(color);
    ...
    if (reader.colorValue("V3DLIGHTS","lightcolor",color)) {
        lightcolor = color;
        QDLwindow->lightsdialog->setlightcolor(color);
    ...
    if (reader.colorValue("V3DLIGHTS","specularcolor",color)) {
        specularcolor = color;
        QDLwindow->lightsdialog->setspecularcolor(color);
...
//void glWidget::applyMoleculeSettings(
//    molecule *mol,
...
//    QFont font;
//    font.fromString(
//        settingValue(file, section, "font")
//    );
//    mol->setfont(font);

//    mol->setdrawatomsymbols(
//        settingValue(file, section, "showsymbols").toInt()
//    );

//    mol->setdrawatomindices(
//        settingValue(file, section, "showindices").toInt()
//    );

//    const bool showSelected =
//        settingValue(file, section, "showselect").toInt();

//    mol->setonlyatomactive(showSelected);

//    if (showSelected) {
//        const QStringList activeAtoms =
//            settingList(file, section, "atomactive");

//        for (int i = 0; i < activeAtoms.size(); i++)
//            mol->setatomactive(i, activeAtoms.at(i).toInt());
//    }

//    mol->setlabelsvshift(
//        settingValue(file, section, "vshift").toInt()
//    );

//    if (CHKsettingspos->isChecked()) {
//        QVector3D translation;

//        if (readVectorSetting(
//                file, section, "position", translation)) {
...
//            settingList(file, section, "rotation");
...
//            mol->setrotation(quaternion.normalized());
...
//    if (readColorSetting(
//            file, section, "fontcolor", fontColor)) {
//        mol->setfontcolor(fontColor);
//    }
//}


//bool glWidget::readColorSetting(const std::string &file,
//                                const QString &section,
//                                const QString &key,
//                                QColor &color) const
//{
//    const QStringList values =
//        settingList(file, section, key);
...
//bool glWidget::readVectorSetting(const std::string &file,
//                                 const QString &section,
//                                 const QString &key,
...
//void glWidget::retrieveBackgroundSettings(
//    const std::string &file)
//{
//    QColor color;

//    if (!readColorSetting(
//            file, "V3DBKG", "bkgcolor", color)) {
//        return;
//    }
...
//void glWidget::retrieveBallsAndCylindersSettings(
//    const std::string &file)
//{
//    ballradius =
//        settingValue(file, "V3DBALLS", "ballradius").toFloat();

//    cylradius =
//        settingValue(file, "V3DBALLS", "cylradius").toFloat();
...
//molecule *glWidget::retrieveMoleculeFromSettings(
//    const std::string &file,
//    const QString &section)
//{
//    const QString fileName =
//        settingValue(file, section, "name");
...
//    if (suffix == "xyz")
//        loaded = readxyz(fileName);
//    else if (suffix == "ggbs")
//        loaded = readggbs(fileName);
//    else {
//        QMessageBox::warning(
//            this,
//            tr("DAMQT"),
//            tr("Unsupported molecular geometry format: %1")
//                .arg(fileName)
//        );
...
//    retrievemolecule();
//    return molecules->last();
//}

//void glWidget::retrieveLightsSettings(
//    const std::string &file)
//{
//    QColor color;

//    if (readColorSetting(
//            file, "V3DLIGHTS", "ambientcolor", color)) {
//        ambientcolor = color;
//        QDLwindow->lightsdialog->setambientcolor(color);
...
//    if (readColorSetting(
//            file, "V3DLIGHTS", "lightcolor", color)) {
//        lightcolor = color;
//        QDLwindow->lightsdialog->setlightcolor(color);
...
//    if (readColorSetting(
//            file, "V3DLIGHTS", "specularcolor", color)) {
//        specularcolor = color;
//        QDLwindow->lightsdialog->setspecularcolor(color);
...
//bool glWidget::retrieveMoleculesSettings(const std::string &file)
//{
//    const int moleculeCount =
//        settingValue(file, "V3DMOLECULES", "nummolecs").toInt();

//    for (int i = 0; i < moleculeCount; i++) {
//        const QString section =
//            QString("V3DMOLECULE_%1").arg(i);

//        molecule *mol = retrieveMoleculeFromSettings(
//            file, section);

//        if (!mol)
//            return false;

//        applyMoleculeSettings(mol, file, section);

//        if (CHKsettingssurf->isChecked())
//            retrieveSurfaceSettings(mol, file, section);

//        if (CHKsettingsgrids->isChecked())
//            retrieveGridSettings(mol, file, section);
//    }

//    return true;
//}

//void glWidget::retrieveSurfaceSettings(
//    molecule *mol,
//    const std::string &file,
//    const QString &section)
//{
//    const int surfaceCount =
//        settingValue(file, section, "nsurf").toInt();

//    for (int j = 0; j < surfaceCount; j++) {
//        const QString fileName = settingValue(
//            file,
//            section,
//            QString("surfname_%1").arg(j)
//        );

//        if (!mol->retrievesurf(fileName))
//            continue;

//        auto *surf = mol->surfaces->last();

//        connect(surf,&surface::updatedisplay,
//                mol,&molecule::updatedisplay);

//        surf->setsolidsurf(
//            settingValue(
//                file, section,
//                QString("solid_%1").arg(j)
//            ).toInt()
//        );
...
//void glWidget::retrieveViewportSettings(
//    const std::string &file)
//{
//    const float savedFov =
//        settingValue(file, "V3DVIEWPORT", "fov").toFloat();

//    const float savedNear =
//        settingValue(file, "V3DVIEWPORT", "zNear").toFloat();

//    const float savedFar =
//        settingValue(file, "V3DVIEWPORT", "zFar").toFloat();
...
//QString glWidget::selectSettingsFile()
//{
//    QFileDialog fileDialog(this);
//    fileDialog.setWindowFlags(Qt::WindowStaysOnTopHint);
//    fileDialog.setDirectory(ProjectFolder);
...
//QString glWidget::settingValue(const std::string &file,
//                               const std::string &section,
//                               const std::string &key) const
//{
//    return QString::fromStdString(
//        CIniFile::GetValue(key, section, file)
//    );
//}


//QStringList glWidget::settingList(const std::string &file,
//                                  const QString &section,
//                                  const QString &key) const
//{
//    return settingValue(file, section, key)
//        .split(',', Qt::SkipEmptyParts);
//}
...
//void glWidget::BTNsaveSettings_clicked()
//{
//    const QString fileName = selectSettingsSaveFile();
...
//    if (CHKsettingslights->isChecked())
//        writeLightsSettings(writer);

//    if (CHKsettingsbkg->isChecked())
//        writeBackgroundSettings(writer);

//    if (CHKsettingsballs->isChecked())
//        writeBallsAndCylindersSettings(writer);

//    if (CHKsettingsviewport->isChecked())
//        writeViewportSettings(writer);

//    if (CHKsettingsmolecs->isChecked())
//        writeMoleculesSettings(writer);

//    QString errorMessage;

//    if (!writer.save(&errorMessage)) {
//        QMessageBox::warning(this,tr("DAMQT"),errorMessage);
//    }
...
QString glWidget::selectSettingsSaveFile()
{
    QFileDialog fileDialog(this);
    fileDialog.setWindowFlags(Qt::WindowStaysOnTopHint);
    fileDialog.setDirectory(ProjectFolder);
    ...
    QString fileName = fileDialog.getSaveFileName(
        ...
        tr("Save settings"),
        molpath,
        tr("Settings files (*.3Dsettings);;All files (*)")
    );
    ...
    if (QFileInfo(fileName).suffix().compare(
        ...
        fileName += QStringLiteral(".3Dsettings");
    }

    return fileName;
}

void glWidget::writeLightsSettings(SettingsWriter& writer)
{
    writer.beginSection(QStringLiteral("V3DLIGHTS"));
...
void glWidget::writeBackgroundSettings(SettingsWriter& writer)
{
    writer.beginSection(QStringLiteral("V3DBKG"));

    bkgcolor = window->getBkgColor();

    writer.writeColor(
        QStringLiteral("bkgcolor"),
        bkgcolor
    );
}

void glWidget::writeBallsAndCylindersSettings(
    SettingsWriter& writer)
{
    writer.beginSection(QStringLiteral("V3DBALLS"));

    writer.writeFloat(
        QStringLiteral("ballradius"),
        ballradius
    );

    writer.writeFloat(
        QStringLiteral("cylradius"),
        cylradius
    );
}

void glWidget::writeViewportSettings(SettingsWriter& writer)
{
    writer.beginSection(QStringLiteral("V3DVIEWPORT"));
    ...
    writer.writeFloat(
        QStringLiteral("zNear"),
        zNear
    );
}

void glWidget::writeMoleculesSettings(SettingsWriter& writer)
{
    writer.beginSection(QStringLiteral("V3DMOLECULES"));

    int numberOfMolecules = 0;

    for (molecule* mol : *molecules) {
        if (mol && !mol->getiscluster())
            ++numberOfMolecules;
    }

    writer.writeInt(
        QStringLiteral("nummolecs"),
        numberOfMolecules
    );

    MoleculeSettingsWriter moleculeWriter(writer);
    ...
    for (molecule* mol : *molecules) {
        ...
        const QString section =
            QStringLiteral("V3DMOLECULE_%1").arg(savedIndex);

        moleculeWriter.writeMolecule(
            mol,
            section,
            CHKsettingspos->isChecked(),
            CHKsettingsrot->isChecked(),
            CHKsettingssurf->isChecked(),
            CHKsettingsgrids->isChecked(),
            CHKsettingsisosurf->isChecked()
        );
...
void glWidget::updateWindowDialog(MyDockWidget *gdock){
...
//    mol->loadcharges(znuc);
//    mol->loadxyz(xyz);
//    mol->setpath(molpath);
//    // mol->setrotation(rotation.normalized());
//    mol->setvisible(true);
//    mol->initatomactive(false);
//    mol->initatomvisible(true);

//    auto *charges = new QVector<double>(mol->getnumatoms(), 0.0);
//    chargescanvas->append(charges);
...
//    const int moleculeCount = molecules->length();

//    QDLwindow->mespimizerdialog->setSPBhostmax(moleculeCount);
//    QDLwindow->mespimizerdialog->setSPBtemplatemax(moleculeCount);

//    if (moleculeCount > 1
//            && QDLwindow->mespimizerdialog->gettemplate() == 1) {
//        templateindex = 2;
//        QDLwindow->mespimizerdialog->setSPBtemplate(templateindex);
//    }
...
bool glWidget::addmolecule()
{
    ...
    /*
     * loadmoleculargeometry() parece trabajar sobre la última molécula
     * de la lista, por lo que debe añadirse antes de llamarla.
     */
    molecules->append(mol);

    if (!loadmoleculargeometry()) {
        delete molecules->takeLast();
        return false;
    }
    ...
    QVector<double> *charges =
        new QVector<double>(mol->getnumatoms(), 0.0);
...
void glWidget::addmoleculeTowindow()
{
    if (!molecules->isEmpty() && molecules->last()->getiscluster()) {
        ...
        msgBox.setInformativeText(
            tr("The last system is a cluster.\n"
               "You must delete it before adding a new molecule.")
        );
        msgBox.setIcon(QMessageBox::Warning);
        msgBox.exec();
    ...
    mol->setname(molname);
    mol->setfullname(fullmolname);
    mol->setballradius(ballradius);
    mol->setcylradius(cylradius);
    mol->setdisthressq(disthressq);
    mol->setscaleradii(scaleradii);
    mol->makeStructureBondsandSticks();
...
//  Function connectMoleculeSignals: prepares connections of a new molecule
//
void glWidget::connectMoleculeSignals(molecule *mol)
{
    if (!mol)
        return;

    connect(mol,&molecule::updatedisplay,
            this,&glWidget::updatedisplay);

    connect(mol,&molecule::updateGL,
            this,&glWidget::updateGL);

    connect(mol,&molecule::animate,
            this,&glWidget::animate);
}
//  End of function connectMoleculeSignals
//  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------


//  Function initializeMolecule: initializes a new molecule
//
void glWidget::initializeMolecule(molecule *mol, bool restoreRotation)
{
    if (!mol)
        return;
    ...
    connectMoleculeSignals(mol);
}
//  End of function initializeMolecule
//  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//    qDebug() << "ProjectFolder:" << ProjectFolder;
        ...
        QString initialDirectory = ProjectFolder;

        if (initialDirectory.startsWith(
                QStringLiteral("file:"),
                Qt::CaseInsensitive)) {
            initialDirectory = QUrl(initialDirectory).toLocalFile();
        }

        initialDirectory = QDir::cleanPath(initialDirectory);
...
void glWidget::rebuildMoleculeNames()
{
    molecule_names.clear();

    for (molecule* mol : *molecules) {
        if (mol && !mol->getiscluster())
            molecule_names.append(mol->getname());
    }
}
...
//  End of function retrievemolecule
//  ---------------------------------------------------------------------------------------------------------------------------
