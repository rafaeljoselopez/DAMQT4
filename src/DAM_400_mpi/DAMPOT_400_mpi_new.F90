...
!    End of namelist defaults
    ...
    if (myrank .eq. 0) then
        ...
        if (lexact) then
            write(6,"(//'This version of the program is not prepared for exact potential. Run DAMPOT_400_mpi instead.',//)")
            abort = 1
        endif
    endif
    ...
    if (myrank .eq. 0) then
        call leedamqtpotpr0        !  Reads files _2016.damqt and _2016.dmqtv (generated by DAM2016) and make chuncks for processors
    endif
    CALL MPI_REDUCE(abort,abortroot,1,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(abortroot,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
    if (abortroot .gt. 0) then
...
!    Tabulates specific points if required
    if (lpoints .and. myrank .eq. 0) then
        call leedamqtpot
        if (largo) then       ! Long-range potential
            if (lgradient) then
    ...
    if (myrank .eq. 0) then
        call system ("rm -f "//trim(trim(projectname)//"_2016.damqt_*"))
        call system ("rm -f "//trim(trim(projectname)//"_2016.dmqtv_*"))
    endif
...
!
  subroutine leedamqtpotpr0
    ...
    USE PARALELO
    implicit none
    integer(KINT) :: i, ia, icarga, icflm, icfposaux, ierr, indnf, indng, interv, iuni
    integer(KINT) :: j, jshft, k, k1, k2, knt, kntcen, kntlm, kntproc
    integer(KINT) :: l, lenindintrv, lm, m, nbasis, nbaspr, ncapspr, ncenant, ncenbas, ncflm, nfant, ngfinant, nsamples, nsize
    integer(KINT), allocatable :: ncenpr(:)
    ...
    allocate(ncenpr(0:nprocs-1), stat = ierr)
    if (ierr .ne. 0) then
    ...
    k = ncen / nprocs
    ncenpr = k
    do i = 1, ncen-k*nprocs
        ncenpr(i-1) = ncenpr(i-1) + 1
    enddo
    
    do i = 0, nprocs-1
        iuni = 30+2*i
        write(strbux,'(i4.2)') i
        strfile = trim(trim(projectname)//"_2016.damqt_"//trim(adjustl(strbux)))
        
#if _WIN32
        open (unit=iuni, file=strfile, form='binary', action = 'write', carriagecontrol='NONE', iostat=ierr)
#elif __INTEL_COMPILER
        open (unit=iuni, file=strfile, form='binary', action = 'write', carriagecontrol='NONE', iostat=ierr)
#else
        open (unit=iuni, file=strfile, form='unformatted', action = 'write', access='stream', iostat=ierr)
#endif
        if (ierr .ne. 0) then
                write(6,"('Cannot open file ', a, ' in processor ',i3)") strfile, myrank
                abort = 1
                return
        endif
        strfile = trim(trim(projectname)//"_2016.dmqtv_"//trim(adjustl(strbux)))
        iuni = iuni + 1
#if _WIN32
        open (unit=iuni, file=strfile, form='binary', action = 'write', carriagecontrol='NONE', iostat=ierr)
#elif __INTEL_COMPILER
...
!        Allocates memory for the basis set
        ...
        do ia = 1, ncen
            ...
            do k = ngini(ia), ngfin(ia)
                k1 = k1 + 1
                nbaspr = nbaspr + 2*ll(k1) + 1
                ncapspr = ncapspr + 1
            enddo
            if (ia+1 .gt. ncenpr(kntproc)+ncenant) then
                write(30+2*kntproc) kntcen, nbaspr, ncapspr
! write(15+kntproc,*) kntcen, nbaspr, ncapspr
! call flush(15+kntproc)
                do j = ncenant+1, ia
                    write(30+2*kntproc) rcen(1,j), rcen(2,j), rcen(3,j), zn(j)
! write(15+kntproc,*) rcen(1,j), rcen(2,j), rcen(3,j), zn(j)
! call flush(15+kntproc)
                enddo
                write(30+2*kntproc) lsto
! write(15+kntproc,*) lSTO
! call flush(15+kntproc)
                do j = ncenant+1, ia
                    write(30+2*kntproc) ngini(j)-ngfinant, ngfin(j)-ngfinant
! write(15+kntproc,*) ngini(j)-ngfinant, ngfin(j)-ngfinant
                    do k = ngini(j), ngfin(j)
                        i = i + 1
                        write(30+2*kntproc)  nf(i)-nfant, nn(i), ll(i), xx(i)
! write(15+kntproc,*) nf(i)-nfant, nn(i), ll(i), xx(i)
! call flush(15+kntproc)
                    enddo
                enddo
                if (ia .ge. ncen) exit
...
!write(15+kntproc,*) lSTO
!call flush(15+kntproc)
                write(30+2*kntproc) nprimitot
!write(15+kntproc,*) nprimitot
!call flush(15+kntproc)
                do j = ncenant+1, ia
                    write(30+2*kntproc) ncontr(j)
!write(15+kntproc,*) ncontr(j)
!call flush(15+kntproc)
                    do k = 1, ncontr(j)
                        knt = knt + 1
                        write(30+2*kntproc) nprimit(knt), ll(knt)
!write(15+kntproc,*) nprimit(knt), ll(knt)
!call flush(15+kntproc)
                        write(30+2*kntproc) xxg(icarga+1:icarga+nprimit(knt))
!write(15+kntproc,"('xxg = ', 10(1x,e12.5))") xxg(icarga+1:icarga+nprimit(knt))
!call flush(15+kntproc)
                        write(30+2*kntproc) cfcontr(icarga+1:icarga+nprimit(knt))
!write(15+kntproc,"('cfcontr = ', 10(1x,e12.5))") cfcontr(icarga+1:icarga+nprimit(knt))
!call flush(15+kntproc)
...
!    Data of density representation
    ...
    kntproc = 0
    ncenant = 0
    write(30+2*kntproc) lmaxexp
! write(15+kntproc,"('lmaxexp = ', i3)") lmaxexp
    do ia = 1, ncen      ! Do over centers
        if (ngini(ia) .le. 0) cycle
        read(10) icfposd(1:lmtop*nintervaj+1,ia)
        if (ia .gt. 1) then
            k = icfposd(lmtop*nintervaj+1,ia-1) - 1
        endif
        icfposd(1:lmtop*nintervaj+1,ia) = icfposd(1:lmtop*nintervaj+1,ia) + k
...
!     fitting coeficients
        ...
        write(30+2*kntproc) icfposd(1:lmtop*nintervaj+1,ia) - k
        write(30+2*kntproc) xajustd(1:nintervaj,ia)
        write(30+2*kntproc) cfajust(icfposd(1,ia):icfposd(lmtop*nintervaj+1,ia)-1)
! write(15+kntproc,"('ia = ', i5,3x, 20(1x,i6))") ia, icfposd(1:lmtop*nintervaj+1,ia) - k
! write(15+kntproc,"('xajust = ', 10(1x,e12.5))") xajustd(1:nintervaj,ia)
! write(15+kntproc,"('cfajust = ',10(1x,e12.5))") cfajust(icfposd(1,ia):icfposd(lmtop*nintervaj+1,ia)-1)
        if (ia+1 .gt. ncenpr(kntproc)+ncenant) then
            if (ia .ge. ncen) exit
            kntproc = kntproc + 1
            write(30+2*kntproc) lmaxexp
...
!    Reads auxiliary integrals from file .dmqtv
    ...
    allocate(cfrint1(icfposd(lmtop*nintervaj+1,ncen)-1), cfrint2l2(icfposd(lmtop*nintervaj+1,ncen)-1), &
            Qgpart(nintervaj*lmtop), qppart(nintervaj*lmtop), rmultip(lmtop,ncen), stat = ierr)
    if (ierr .ne. 0) then
...
!    multipolar moments
        ...
        write(30+2*kntproc+1) rmultip(1:lmtop,ia)
! write(15+kntproc+nprocs,"('rmultip = ', 10(1x,e12.5))") rmultip(1:lmtop,ia)
!    Reads the integrals:
!        Qg(la,ma,i;ia) = 
...
!    where  ia  labels the center (atom),
        ...
        write(30+2*kntproc+1) Qgpart(1:nintervaj*lmtop)
! write(15+kntproc+nprocs,"('Qgpart = ', 10(1x,e12.5))") Qgpart(1:nintervaj*lmtop)
        read(11) qppart(1:nintervaj*lmtop)
        write(30+2*kntproc+1) qppart(1:nintervaj*lmtop)
! write(15+kntproc+nprocs,"('qppart = ', 10(1x,e12.5))") qppart(1:nintervaj*lmtop)
!    Reads the fitting coeficients of auxiliary integrals for electrostatic potential and field
        ...
        write(30+2*kntproc+1) cfrint1(icfposd(1,ia):icfposd(lmtop*nintervaj+1,ia)-1)
! write(15+kntproc+nprocs,"('cfrint1 = ', 10(1x,e12.5))") cfrint1(icfposd(1,ia):icfposd(lmtop*nintervaj+1,ia)-1)
        read(11) cfrint2l2(icfposd(1,ia):icfposd(lmtop*nintervaj+1,ia)-1)    ! Expansion coefficients of auxiliary integrals rint2l2
        write(30+2*kntproc+1) cfrint2l2(icfposd(1,ia):icfposd(lmtop*nintervaj+1,ia)-1)
! write(15+kntproc+nprocs,"('cfrint2l2 = ', 10(1x,e12.5))") cfrint2l2(icfposd(1,ia):icfposd(lmtop*nintervaj+1,ia)-1) 
        if (ia+1 .gt. ncenpr(kntproc)+ncenant) then
            if (ia .ge. ncen) exit
            kntproc = kntproc + 1
            ncenant = ia
    ...
    deallocate (atmnam, cfajust, cfrint1, cfrint2l2, icfposd, ll, lmaxc, ncenpr, nf, ngini, ngfin, nzn, Qgpart, qppart, &
        rcen, rmultip, rnor, xajustd, zn, stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when deallocating atmnam, cfajust, cfrint1, cfrint2l2,...  in processor ',i3)") myrank
        abort = 1
        return
    endif
    if (lsto) then
        deallocate(nn, xx, stat = ierr)
        if (ierr .ne. 0) then
            write(6,"('Memory error when deallocating atmnam, nn and xx in processor ',i3)") myrank
            abort = 1
            return
        endif
    else
        deallocate(cfcontr, ipntprim, ncontr, nprimit, xxg, stat = ierr)
        if (ierr .ne. 0) then
            write(6,"('Memory error when deallocating atmnam, cfcontr, ipntprim, ncontr, nprimit, xxg in processor ',i3)") myrank
            abort = 1
            return
        endif
    endif
...
!
  subroutine leedamqtpotprocs
    ...
    strfile = trim(trim(projectname)//"_2016.damqt_"//trim(adjustl(strbux)))
    inquire(file=strfile, size=nsize, iostat=ierr)
    if (ierr .ne. 0) then
        write(6,"('Error when inquiring file ', a, ' in processor ',i3)") strfile, myrank
        abort = 1
        return
    endif
    if (nsize .eq. -1) then
        write(6,"('Size of file ', a, ' cannot be determined in processor ',i3)") strfile, myrank
        abort = 1
        return
    endif

#if _WIN32
    open (unit=10, file=strfile, form='binary', action = 'read', carriagecontrol='NONE', iostat=ierr)
#elif __INTEL_COMPILER
    open (unit=10, file=strfile, form='binary', action = 'read', carriagecontrol='NONE', iostat=ierr)
#else
    open (unit=10, file=strfile, form='unformatted', action = 'read', access='stream', iostat=ierr)
#endif
    if (ierr .ne. 0) then
            write(6,"('Cannot open file ', a, ' in processor ',i3)") strfile, myrank
            abort = 1
    ...
    strfile = trim(trim(projectname)//"_2016.dmqtv_"//trim(adjustl(strbux)))
#if _WIN32
    open (unit=11, file=strfile, form='binary', action = 'read', carriagecontrol='NONE', iostat=ierr)
#elif __INTEL_COMPILER
    open (unit=11, file=strfile, form='binary', action = 'read', carriagecontrol='NONE', iostat=ierr)
#else
    open (unit=11, file=strfile, form='unformatted', action = 'read', access='stream', iostat=ierr)
#endif
    if (ierr .ne. 0) then
        write(6,"('Cannot open file ', a, ' in processor ',i3)") strfile, myrank
        abort = 1
...
!	Allocates memory for geometry
    ...
    if (allocated(atmnam)) then
        deallocate(atmnam, nzn, rcen, zn)
    endif
    allocate(atmnam(ncen), nzn(ncen), rcen(3,ncen), zn(ncen), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating atmnam, nzn, rcen and zn in processor ',i3)") myrank
...
!		Allocates memory for the basis set
        ...
        if (allocated(ll)) then
            deallocate(ll, lmaxc, nf, ngini, ngfin, nn, rnor, xx)
        endif
        allocate(ll(ncaps), lmaxc(ncen), nf(ncaps), ngini(ncen), ngfin(ncen), nn(ncaps), rnor(ncaps), &
                xx(ncaps), stat = ierr)
        if (ierr .ne. 0) then
...
!		Allocates memory for the basis set
        ...
        if (allocated(cfcontr)) then
            deallocate(cfcontr, ipntprim, ll, lmaxc, ncontr, nf, ngini, ngfin, nprimit, rnor, xxg)
        endif
        allocate(cfcontr(nprimitot), ipntprim(ncaps), ll(ncaps), lmaxc(ncen), ncontr(ncen), nf(ncaps), ngini(ncen), &
                ngfin(ncen), nprimit(ncaps), rnor(ncaps), xxg(nprimitot), stat = ierr)
        if (ierr .ne. 0) then
...
!	Data of density representation
    ...
    if (allocated(icfposd)) then
        deallocate(icfposd, xajustd, cfajust)
    endif
    allocate(icfposd(lmtop*nintervaj+1,ncen), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating icfposd in processor ',i3)") myrank
...
!	Reads auxiliary integrals from file .dmqtv
    ...
    if (allocated(cfrint1)) then
        deallocate(cfrint1, cfrint2l2, QGacum, Qgpart, qpacum, qppart, rlargo, rmultip)
    endif
    allocate(cfrint1(icfposd(lmtop*nintervaj+1,ncen)-1), cfrint2l2(icfposd(lmtop*nintervaj+1,ncen)-1), &
            QGacum(nintervaj*lmtop,ncen), Qgpart(nintervaj*lmtop), qpacum(nintervaj*lmtop,ncen), qppart(nintervaj*lmtop), &
            rlargo(ncen), rmultip(lmtop,ncen), stat = ierr)
...
!	Determines the long-range radii and the highest l in the expansion for each interval
    if (allocated(lcorto)) then
        deallocate(lcorto, llargo, Qllargo)
    endif
    allocate(lcorto(nintervaj,ncen), llargo(0:mxlargo,ncen), Qllargo(0:lmaxrep), stat = ierr )
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating lcorto, llargo, auxll,  and Qllargo in processor ',i3)") myrank
...
!
   subroutine mespgrid(tarray)
    ...
    integer(KINT) :: i, ia, ierr, iii, isel, iuni, ix, iy, iz, knt, nx, ny, nz
    ...
    real(KREAL) :: x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2)
#else
    real(KREAL4) :: x1, x2, y1, y2, z1, z2
    real(KREAL4) :: x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2)
#endif
...
!    Opens file for electrostatic potential tabulation
    ...
    iuni = 21
    strfile = trim(trim(filename)//"-v.plt_"//trim(adjustl(strbux)))
    call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
    array = cero
!    Opens files for electrostatic potential gradient tabulation
    if (lgradient) then
        ...
        iuni = 23
        strfile = trim(trim(filename)//"-v-dx.pltd_"//trim(adjustl(strbux)))
        call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
        iuni = 24
        strfile = trim(trim(filename)//"-v-dy.pltd_"//trim(adjustl(strbux)))
        call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
        iuni = 25
        strfile = trim(trim(filename)//"-v-dz.pltd_"//trim(adjustl(strbux)))
        call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
...
!    Opens files for electrostatic potential second derivatives tabulation
    if (lderiv2) then
        allocate(arraydxx(nx), arraydxy(nx), arraydxz(nx), arraydyy(nx), arraydyz(nx) &
                , arraydzz(nx), stat = ierr)
        if (ierr .ne. 0) then
        ...
        iuni = 28
        strfile = trim(trim(filename)//"-v-dxx.pltd_"//trim(adjustl(strbux)))
        call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
        iuni = 29
        strfile = trim(trim(filename)//"-v-dxy.pltd_"//trim(adjustl(strbux)))
        call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
        iuni = 30
        strfile = trim(trim(filename)//"-v-dxz.pltd_"//trim(adjustl(strbux)))
        call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
        iuni = 31
        strfile = trim(trim(filename)//"-v-dyy.pltd_"//trim(adjustl(strbux)))
        call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
        iuni = 32
        strfile = trim(trim(filename)//"-v-dyz.pltd_"//trim(adjustl(strbux)))
        call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
        iuni = 33
        strfile = trim(trim(filename)//"-v-dzz.pltd_"//trim(adjustl(strbux)))
        call cabecera(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, strfile)
        ...
        do iy = 1, ny
            ...
            do ix =1, nx
                ...
                do ia = 1, ncen
                    ...
                    array(knt) = array(knt) + vtot
                    if (lgradient) then
                        arraydx(knt) = arraydx(knt) + drvx
                        arraydy(knt) = arraydy(knt) + drvy
                        arraydz(knt) = arraydz(knt) + drvz
                    endif
                    if (lderiv2) then
                        arraydxx(knt) = arraydxx(knt) + dxx
                        arraydxy(knt) = arraydxy(knt) + dxy
                        arraydxz(knt) = arraydxz(knt) + dxz
                        arraydyy(knt) = arraydyy(knt) + dyy
                        arraydyz(knt) = arraydyz(knt) + dyz
                        arraydzz(knt) = arraydzz(knt) + dzz
                    endif
                enddo
            enddo
            call linedbl( 21, nx , array )
            if (lgradient) then
                call linedbl(23, nx , arraydx )
                call linedbl(24, nx , arraydy )
                call linedbl(25, nx , arraydz )
            endif
            if (lderiv2) then
                call linedbl(28, nx , arraydxx )
                call linedbl(29, nx , arraydxy )
                call linedbl(30, nx , arraydxz )
                call linedbl(31, nx , arraydyy )
                call linedbl(32, nx , arraydyz )
                call linedbl(33, nx , arraydzz )
            endif
        enddo
    enddo
    if (myrank .ne. 0) then
...
!     Deallocates arrays and closes the grid files
...
#ifdef DBLPRCGRID
    if (nprocs .eq. 1) then
        call system ("mv "//trim(filename)//"-v.plt_00 "//trim(filename)//"-v.plt ")
        if (lgradient) then
            call system ("mv "//trim(filename)//"-v-dx.pltd_00 "//trim(filename)//"-v-dx.pltd ")
            call system ("mv "//trim(filename)//"-v-dy.pltd_00 "//trim(filename)//"-v-dy.pltd ")
            call system ("mv "//trim(filename)//"-v-dz.pltd_00 "//trim(filename)//"-v-dz.pltd ")
        endif
        if (lderiv2) then
            call system ("mv "//trim(filename)//"-v-dxx.pltd_00 "//trim(filename)//"-v-dxx.pltd ")
            call system ("mv "//trim(filename)//"-v-dxy.pltd_00 "//trim(filename)//"-v-dxy.pltd ")
            call system ("mv "//trim(filename)//"-v-dxz.pltd_00 "//trim(filename)//"-v-dxz.pltd ")
            call system ("mv "//trim(filename)//"-v-dyy.pltd_00 "//trim(filename)//"-v-dyy.pltd ")
            call system ("mv "//trim(filename)//"-v-dyz.pltd_00 "//trim(filename)//"-v-dyz.pltd ")
            call system ("mv "//trim(filename)//"-v-dzz.pltd_00 "//trim(filename)//"-v-dzz.pltd ")
        endif
        deallocate(array)
        tiempo = dtime(tarray)
        return
    ...
    deallocate(array)
    if (myrank .eq. 0) then
        write(6,"('Total number of long-range contributions  = ',6x, i15)") kntlargotot
        write(6,"('Total number of short-range contributions = ',6x, i15)") kntcortotot
        write(6,"('Total number of tabulated points = ', i15)") nx*ny*nz
        do i = 0, nprocs-1
            iuni = 20 + i
            write(strbux,'(i4.2)') i
            strfile = trim(trim(filename)//"-v.plt_"//trim(adjustl(strbux)))
#if _WIN32
            open (unit=iuni, file=strfile, form='binary', carriagecontrol='NONE', iostat=ierr)
#elif __INTEL_COMPILER
            open (unit=iuni, file=strfile, form='binary', carriagecontrol='NONE', iostat=ierr)
#else
            open (unit=iuni, file=strfile, form='unformatted', access='stream', iostat=ierr)
#endif
            if (ierr .ne. 0) then
                write(6,"('Error ', i5, ' opening file ', a, ' in mespgrid in processor ',i3)") ierr, strfile, myrank
                abort = 1
                return
            endif
        enddo
        write(strbux,'(i4.2)') 0
        strfile = trim(trim(filename)//"-v.plt_"//trim(adjustl(strbux)))
        call readheader(nx, ny, nz, x1v(1), x2v(1), y1v(1), y2v(1), z1v(1), z2v(1), 20, strfile, ierr)
!         MESP -----------------
        call addmesp(nx, ny, nz, x1v, x2v, y1v, y2v, z1v, z2v)
        if (abort .eq. 1) return
!         MESP GRADIENT -----------------
        if (lgradient) then
            call addmespgrad(nx, ny, nz, x1v, x2v, y1v, y2v, z1v, z2v)
        endif
!         MESP SECOND DERIVATIVES -----------------
        if (lderiv2) then
            call addmespderiv2(nx, ny, nz, x1v, x2v, y1v, y2v, z1v, z2v)
        endif
        tiempo = dtime(tarray)
    endif
    return
...
!   ***************************************************************
!
   subroutine addmesp(nx, ny, nz, x1v, x2v, y1v, y2v, z1v, z2v)
    USE MPI
    USE DAM_400_D
    USE DAMPOT_400_D
    USE DAM_400_CONST_D
    ...
    USE PARALELO
    implicit none
    integer(KINT) :: i, ierr, iuni, nx, nx2, ny, ny2, nz, nz2, testcomp
    character*4 :: strbux
    character*256 :: strfile
    real(KREAL), allocatable :: array(:), array2(:)
    ...
    real(KREAL4) :: x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2)
#endif
!         Checks the compatibility of mesp grids
    do i = 1, nprocs-1
        write(strbux,'(i4.2)') i
        strfile = trim(trim(filename)//"-v.plt_"//trim(adjustl(strbux)))
        call readheader(nx2, ny2, nz2, x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2), 20+i, strfile, ierr)
        abort = testcomp(ierr, nx, ny, nz, nx2, ny2, nz2, x1v, x2v, y1v, y2v, z1v, z2v)
        if (abort .ne. 0) return
    enddo
    allocate(array(nx*ny*nz), array2(nx*ny*nz), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating array and array2 in addmesp in processor ',i3)") myrank
        abort = 1
        return
    endif
    strfile = trim(trim(filename)//"-v.plt")
    iuni = 20 + nprocs
#if _WIN32
    open (unit=iuni, file=strfile, form='binary', carriagecontrol='NONE', iostat=ierr)
#elif __INTEL_COMPILER
...
#endif          
    if (ierr .ne. 0) then
        write(6,"('Error ', i5, ' opening file ', a, ' in addmespgrad in processor ',i3)") ierr, strfile, myrank
        abort = 1
        return
    endif
!     Accumulates MESP contributions from processors and writes to file
    call cabecera(nx, ny, nz, x1v(1), x2v(1), y1v(1), y2v(1), z1v(1), z2v(1), iuni, strfile)
    array = cero
    do i = 0, nprocs-1
        iuni = 20 + i
        call readlinedbl(iuni,nx*ny*nz,array2)
        array = array + array2
    enddo
#ifdef DBLPRCGRID
    call linedbl(20 + nprocs,nx*ny*nz,array)
#else
    allocate(arraysp(nx*ny*nz), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating array and arraysp in addmesp in processor ',i3)") myrank
        abort = 1
        return
    endif
    arraysp = array
    call linea(20 + nprocs,nx*ny*nz,arraysp)
#endif 
    do i = 0, nprocs-1
        iuni = 20 + i
        close(iuni)
        write(strbux,'(i4.2)') i
        strfile = trim(trim(filename)//"-v.plt_"//trim(adjustl(strbux)))
        call system ("rm -f "//trim(strfile))
    enddo
    close(20 + nprocs)
    deallocate(array,array2)
    if (allocated(arraysp)) deallocate(arraysp)
    return
    end
!    
!   ***************************************************************
!
    function testcomp(ierr, nx, ny, nz, nx2, ny2, nz2, x1v, x2v, y1v, y2v, z1v, z2v) result(failed)
    USE DAM_400_D
    integer(KINT), intent(in) :: ierr, nx, ny, nz, nx2, ny2, nz2
    integer(KINT) :: failed
#ifdef DBLPRCGRID
    real(KREAL), intent(in) :: x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2)
#else
    real(KREAL4), intent(in) :: x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2)
#endif
    if (ierr .ne. 0 .or. nx .ne. nx2 .or. ny .ne. ny2 .or. nz .ne. nz2 &
        .or. abs(x1v(1)-x1v(2)) .gt. 1.e-5*abs(x1v(1)) .or. abs(x2v(1)-x2v(2)) .gt. 1.e-5*abs(x2v(1)) &
        .or. abs(y1v(1)-y1v(2)) .gt. 1.e-5*abs(y1v(1)) .or. abs(y2v(1)-y2v(2)) .gt. 1.e-5*abs(y2v(1)) &
        .or. abs(z1v(1)-z1v(2)) .gt. 1.e-5*abs(z1v(1)) .or. abs(z2v(1)-z2v(2)) .gt. 1.e-5*abs(z2v(1)) ) then
        if (ierr .eq. 0) then
            write(6,"('Incompatible grids: ',/,'Grid(0): nx = ', i4, ' ny = ', i4, ' nz = ', i4, &
                / 'x1 = ', e12.5, ' x2 = ', e12.5, ' y1 = ', e12.5, ' y2 = ', e12.5, ' z1 = ', e12.5, ' z2 = ', e12.5, &
                /,' Grid2(',i3,'): nx = ', i4, ' ny = ', i4, ' nz = ', i4, &
                / 'x1 = ', e12.5, ' x2 = ', e12.5, ' y1 = ', e12.5, ' y2 = ', e12.5, ' z1 = ', e12.5, ' z2 = ', e12.5)") &
                nx, ny, nz, x1v(1), x2v(1), y1v(1), y2v(1), z1v(1), z2v(1), &
                i, nx2, ny2, nz2, x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2)
        endif
        failed = 1
        return
    endif
    failed = 0
    return
    end function testcomp
!    
!   ***************************************************************
!
   subroutine addmespgrad(nx, ny, nz, x1v, x2v, y1v, y2v, z1v, z2v)
    USE MPI
    USE DAM_400_D
    USE DAMPOT_400_D
    USE DAM_400_CONST_D
    ...
    USE PARALELO
    implicit none
    integer(KINT) :: failed, i, ierr, iuni, j, nx, nx2, ny, ny2, nz, nz2, testcomp
    character*4 :: strbux
    character*256 :: strfile
    character(2) :: tipo(0:2) = (/ "dx", "dy", "dz" /)
    real(KREAL), allocatable :: array(:), array2(:)
    real(KREAL4), allocatable :: arraysp(:)
#ifdef DBLPRCGRID
    real(KREAL) :: x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2)
...
!         Checks the compatibility of mesp grids
    do i = 0, nprocs-1
        ...
        do j = 0, 2
            write(strbux,'(i4.2)') i
            strfile = trim(trim(filename)//"-v-"//tipo(j)//".pltd_"//trim(adjustl(strbux)))
            call readheader(nx2, ny2, nz2, x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2), 20+3*i+j, strfile, ierr)
            failed = failed + testcomp(ierr, nx, ny, nz, nx2, ny2, nz2, x1v, x2v, y1v, y2v, z1v, z2v)
        enddo
        if (failed .ne. 0) return
    enddo
    allocate(array(nx*ny*nz), array2(nx*ny*nz), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating array and array2 in addmespgrad in processor ',i3)") myrank
        abort = 1
        return
    endif
    do j = 0, 2
        strfile = trim(trim(filename)//"-v-"//tipo(j)//".pltd")
        iuni = 20 + 3*nprocs
#if _WIN32
        open (unit=iuni, file=strfile, form='binary', carriagecontrol='NONE', iostat=ierr)
#elif __INTEL_COMPILER
...
!     Accumulates MESP contributions from processors and writes to file
        ...
        enddo
#ifdef DBLPRCGRID
    call linedbl(20 + 3*nprocs,nx*ny*nz,array)
#else
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating arraysp in addmespgrad in processor ',i3, ' dim = ', i12)") myrank, nx*ny*nz
        abort = 1
        return
    endif
    arraysp = array
    call linea(20 + 3*nprocs,nx*ny*nz,arraysp)
#endif
        do i = 0, nprocs-1
            iuni = 20 + 3*i+j
            close(iuni)
...
!   ***************************************************************
!
   subroutine addmespderiv2(nx, ny, nz, x1v, x2v, y1v, y2v, z1v, z2v)
    USE MPI
    USE DAM_400_D
    USE DAMPOT_400_D
    USE DAM_400_CONST_D
    ...
    character*4 :: strbux
    character*256 :: strfile
    character(3) :: tipo(0:5) = (/ "dxx", "dxy", "dxz", "dyy", "dyz", "dzz" /)
    real(KREAL), allocatable :: array(:), array2(:)
    real(KREAL4), allocatable :: arraysp(:)
...
!         Checks the compatibility of mesp grids
    do i = 0, nprocs-1
        ...
        do j = 0, 5
            write(strbux,'(i4.2)') i
            strfile = trim(trim(filename)//"-v-"//tipo(j)//".pltd_"//trim(adjustl(strbux)))
            call readheader(nx2, ny2, nz2, x1v(2), x2v(2), y1v(2), y2v(2), z1v(2), z2v(2), 20+6*i+j, strfile, ierr)
            failed = failed + testcomp(ierr, nx, ny, nz, nx2, ny2, nz2, x1v, x2v, y1v, y2v, z1v, z2v)
        enddo
        if (failed .ne. 0) return
    enddo
    allocate(array(nx*ny*nz), array2(nx*ny*nz), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating array and array2 in addmespderiv2 in processor ',i3)") myrank
        abort = 1
        return
    endif
    do j = 0, 5
...
#endif         
        if (ierr .ne. 0) then
            write(6,"('Error ', i5, ' opening file ', a, ' in addmespderiv2 in processor ',i3)") ierr, strfile, myrank
            abort = 1
            return
        endif
...
!     Accumulates MESP contributions from processors and writes to file
        ...
        enddo
#ifdef DBLPRCGRID
    call linedbl(20 + 6*nprocs,nx*ny*nz,array)
#else
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating array and arraysp in addmespderiv2 in processor ',i3)") myrank
        abort = 1
        return
    endif
    arraysp = array
    call linea(20 + 6*nprocs,nx*ny*nz,arraysp)
#endif
        do i = 0, nprocs-1
            iuni = 20 + 6*i+j
            close(iuni)
...
!
  subroutine emes ( m1, m2, ms, md, ss, sd )
    USE DAM_400_D
    USE DAM_400_CONST_D, ONLY: uno, cero, umed
    implicit none
...
#if _WIN32
    open (unit=iuni, file=s, form='binary', carriagecontrol='NONE', iostat=ierr)
#elif __INTEL_COMPILER
    open (unit=iuni, file=s, form='binary', carriagecontrol='NONE', iostat=ierr)
#else
    open (unit=iuni, file=s, form='unformatted', access='stream', iostat=ierr)
#endif
    if (ierr .ne. 0) then
        write(6,"('Error ', i5, ' openening file ', a)") ierr, s
    endif
...
!
!    ***************************************************************
!    Subroutine readheader: reads header of .plt files (binary)
!
  subroutine readheader(nx, ny, nz, x1, x2, y1, y2, z1, z2, iuni, s, ierr)
    USE DAM_400_D
    implicit none
    integer(KINT) :: i, ierr
    integer(KINT) :: nx,ny,nz,iuni,ns,iaux(0:2)
...
!    sentence for stream files according to Fortran 2003 standard
    ...
    open (unit=iuni, file=s, form='unformatted', access='stream', iostat=ierr)
#endif
!   i0    ! 3 for single precision grid and gOpenMol compatibility, 0 for double precision grid (no gOpenMol compatibility)
    read(iuni) i0, iaux(1)
#ifdef DBLPRCGRID 
    if (i0 .ne. 0) then
        write(6,"('Double precision grid incompatible with current version of program')")
        write(6,"('Unseal line #define DBLPRCGRID at the top of the file and recompile')") 
        close(iuni)
        ierr = 1
        return
    endif
#else
    if (i0 .ne. 3) then
        write(6,"('Single precision grid incompatible with current version of program')")
        write(6,"('Seal with comment line #define DBLPRCGRID at the top of the file and recompile')") 
        close(iuni)
        ierr = 1
        return
    endif
#endif
    read(iuni) nz, ny, nx
    read(iuni) z1, z2, y1, y2, x1, x2
    end
!
!    ***************************************************************
!    Subroutine linedbl: writes lines of double precision .plt files generated by processors (binary)
!
  SUBROUTINE linedbl(iuni,n,v)
    USE DAM_400_D
    implicit none
    integer(KINT) :: i, iuni, n
    real(KREAL) :: v(0:n-1)
    ...
    END
!
!    Subroutine readlinedbl: reads lines of double precision .plt files generated by processors (binary)
!
  SUBROUTINE readlinedbl(iuni,n,v)
    USE DAM_400_D
    implicit none
    integer(KINT) :: i, iuni, n
    real(KREAL) :: v(n)
...
!
  subroutine error(ierr, msg)
    USE MPI
    USE DAM_400_D
    USE DAM_400_DATA_D, ONLY: projectname
    USE DAMPOT_400_D, ONLY: filename
    implicit none
    integer(KINT) :: i, ierr, ierr2
    character(*) :: msg
    character(2) :: tipod1(0:2) = (/ "dx", "dy", "dz" /)
    character(3) :: tipod2(0:5) = (/ "dxx", "dxy", "dxz", "dyy", "dyz", "dzz" /)
    call system ("rm -f "//trim(trim(projectname)//"_2016.damqt_*"))
    call system ("rm -f "//trim(trim(projectname)//"_2016.dmqtv_*"))
    call system ("rm -f "//trim(trim(filename)//"-v.plt_*"))
    do i = 0, 2
        call system ("rm -f "//trim(trim(filename)//"-v-"//tipod1(i)//".pltd_*"))
    enddo
    do i = 0, 5
        call system ("rm -f "//trim(trim(filename)//"-v-"//tipod2(i)//".pltd_*"))
    enddo
    write(6,"(a)") msg
