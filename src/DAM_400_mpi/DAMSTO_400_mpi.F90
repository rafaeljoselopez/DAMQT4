...
!
! Requires the file DAM_400_GLOBAL.F90 which must be compiled first (contains the modules)
!
...
!
  program DAM_400_mpi
    ...
    USE DAM_400_DATA_D
    USE DAMQT_UTILS
    USE DAM_400_LEGENDRE_D
    USE PARALELO
    implicit none
    logical :: leg35, lprint, lsgbs, lsgbsden, lsgbsgz, lsgbsdengz
    integer(KINT) :: i, ia, icen, idlt, ierr, irank, ishift, ishiftnw, j, k, kj, knt, kntlm
    ...
    logical :: lnamelist(10), ltimeprocs
...
!    Defaults for the NAMELIST OPTIONS
    ...
    ipmax = 40              ! length of the expansion of the B integrals in subroutine frgsigma
...
!        with subroutine leedamm2c otherwise read them with subroutine readsgbsden
        ...
        if (abort .eq. 0) then
            ...
            lnamelist = (/ lsgbs, lden, lsgbsden, leg35, longoutput, iswindows, ltimeprocs, lvalence, lzdo, lm2c /)
        endif
    endif
    CALL MPI_REDUCE(abort,abortroot,1,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(abortroot,1,MPI_LOGICAL,0,MPI_COMM_WORLD,ierr)
    ...
    CALL MPI_BCAST(lnamelist,10,MPI_LOGICAL,0,MPI_COMM_WORLD,ierr)
    ...
    if (myrank .eq. 0)    then
        if (lsgbsden) then
            call readsgbsden(lsgbsden)  ! reads geometry, basis set and density from a single file and writes them to files .sgbs and .den
        else
            call readsgbs
        endif
        if (lzdo) then
    ...
    if (leg35) then    ! Legendre quadrature rule with 35 points
        nqleg = 35
        u => uleg35
        w => wleg35
    endif
    if (ipmax .gt. mxsigexp) then
        if (myrank .eq. 0) write(6,"('Length of expansion of B integrals: ', i5,' higher than maximum allowed: ', i5)") &
                ipmax, mxsigexp
        if (myrank .eq. 0) write(6,"('Sets it to maximum allowed value')")
        ipmax = mxsigexp
    endif
...
!    Reads geometry, basis set generated by an external program

    if (myrank .ne. 0 .or. lsgbsden) then
...
!    Allocates buffers for para_range
    allocate(istav(0:nprocs), iendv(0:nprocs-1), ilenv(0:nprocs-1), idispv(0:nprocs-1), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating istav, iendv, ilenv and idispv in processor ',i8)") myrank
        abort = 1
    endif
    CALL MPI_REDUCE(abort,abortroot,1,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(abortroot,1,MPI_LOGICAL,0,MPI_COMM_WORLD,ierr)
...
!   Reads the density matrix and makes chunks of it for processors

    if (myrank .eq. 0) then
        if (.not. lsgbsden) call readden
        if (abort .eq. 0) then
            call para_range_den
            call totalchargeSTO
        endif
    ...
    if (myrank .eq. 0 .and. longoutput) then
        write(6,"('in proc ', i8, ' istav = ', 20(1x,i6))") myrank, istav(0:nprocs-1)
        write(6,"('in proc ', i8, ' iendv = ', 20(1x,i6))") myrank, iendv(0:nprocs-1)
        write(6,"('in proc ', i8, ' ilenv = ', 20(1x,i6))") myrank, ilenv(0:nprocs-1)
    endif

    istart = istav(myrank)
    iend = iendv(myrank)
    if (longoutput) then
        write(6,"('in proc ', i8, ' istart = ', i6, ' iend = ', i6)") myrank, istart, iend
    endif
...
!     in the subroutine ajusta
  subroutine createdenprocs
    ...
    USE DAM_400_D
    USE DAM_400_CONST_D
    USE DAM_400_DATA_D, kntblock_ori => kntblock
    USE GAUSS
    USE PARALELO, istart_ori => istart, iend_ori => iend
    implicit none
    integer(KINT) :: i, i1, i2, ia, ib, ib2, ierr, irank, istart, iend, iuni, knt, kntblock, ktot, la, lb, ma, mb
    integer(KINT) :: nfa, nfb, nelemden, nga1, nga2, ngb1, ngb2, ngb21, ngb22
    real(KREAL) :: rab, rab2, xa, xab, xab2, xb, xb2, xy, ya, yab, yab2, yb, yb2, za, zab, zab2, zb, zb2
    logical, allocatable :: lblockaux(:), lskip(:)
    character*4 :: strbux

!    Allocates memory for arrays lskip
    allocate(lskip(ncen), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating lskip in processor ',i8)") myrank
        abort = 1
        return
    endif
...
!             Two-center blocks
        ...
        enddo
        if (knt .lt. nelemden) write(iuni) (cero, i = 1, nelemden-knt)  ! completes with zeroes if actual number of elements lower than predicted
...
!
  subroutine para_range_den
    ...
    USE PARALELO
    implicit none
    integer(KINT) :: i, iaux, ierr, ibasedlt, j
...
!
  subroutine readsgbs
    ...
    USE PARALELO
    implicit none
    integer(KINT) :: i, ia, ib, ierr, igrespec, indgr, iopsim, ios, ivclass, ivopsim, j, k, m2cmxcap, m2cmxfun, m2cmxcen
...
!    generated by SMILES. These "extra" data are just skipped)
    ...
    if (ierr .ne. 0) then
        write(6,"('Cannot open file ', a, ' in processor ',i8)") trim(projectname)//".sgbs", myrank
        abort = 1
        return
    endif
    rewind(15)
    ...
    if (myrank .eq. 0 .and. longoutput)  write(6,"('m2cmxcap, m2cmxfun, m2cmxcen = ', 3(1x,i8))")  m2cmxcap, m2cmxfun, m2cmxcen
!    Checks the compatibility of some dimensions with those of SMILES
    if (m2cmxcen .ne. mxcen .and. myrank .eq. 0) then
        write(6,"('WARNING: Parameter  mxcen  has different values in module DAM_400_D than in smiles')")
        write(6,"('Value in DAM_400_D = ',i5)") mxcen
...
!    Allocates memory for geometry and basis set
    ...
    allocate(atmnam(ncen), av(0:2*mxn+4*mxl), bv(0:2*mxn+4*mxl), ll(ncaps), lmaxc(ncen), &
            nf(ncaps), ngini(ncen), ngfin(ncen), nn(ncaps), nzn(ncen), nbasesac(0:ncen), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error ', i5, ' in readsgbs when allocating atmnam, av, bv, ll, lmaxc, nf, ngini, &
            & ngfin, nn, nzn, nbasesac in processor ', i8)") ierr, myrank
        abort = 1
        return
    endif
...
!    Marks the blocks of negligible charge distributions the matrix in array lsdisf
    ...
    if (ioptaj .ne. 2) then
        ...
        if (ierr .ne. 0) then
            write(6,"('Memory error ', i5, ' in readsgbs when allocating lsdisf in processor ', i8)") ierr, myrank
            abort = 1
            return
        endif
        do ia = 1, ncen
...
!    prints out the input data
    if (myrank .eq. 0) then
        ...
        write(6,"(/t1, ' no. of center:', t20, 'x', t32, 'y', t44, 'z', t56, 'charge', t69, 'n. of shells')")
        do ia = 1, ncen
        ...
        write(6,"('Number of basis functions = ', i7)") nbas
    endif
    return
    end
!
...
!
  subroutine readden
    ...
    logical lcmplxgr, ldst, lgrespec
    real(KREAL) :: rijk(3,3)
    if (myrank .ne. 0) return       ! This subroutine must be called only by main processor
...
!    Reads the density matrix as written by the direct minimization program of SMILES
    ...
    read(16, iostat = ierr) nbasis, ((dmat(i,j), i=1,nbasis), j=1,nbasis)
    if ( ierr .ne. 0 .or. nbas .ne. nbasis ) then
        write(6,"('ERROR reading density matrix from file ', a)") trim(projectname)//".den"
        if (nbas .ne. nbasis ) write(6,"('It does not agree with the basis set')")
        abort = 1
        return
    endif
    close(16)
...
!    ***************************************************************
!
  subroutine readsgbsden(lsgbsden)
    USE MPI
    USE DAM_400_D
    USE DAM_400_CONST_D
    USE DAM_400_DATA_D
    ...
    real(KREAL) :: rab
    if (myrank .ne. 0) return   ! This subroutine must be called only by the main processor
    if (lsgbsden) then
        iunit = 17
        open(iunit,file=trim(projectname)//".sgbsden",form='formatted', iostat=ierr)
        if (ierr .ne. 0) then
            write(6,"('File ', a, ' not found, tries with standard input')") trim(projectname)//".sgbsden"
            iunit = 5
        endif
    else
        iunit = 5
...
!    Reads the number of centers
    read(iunit, *, iostat = ierr) ncen
    if (ierr .ne. 0) then
        write(6,"('Error reading number of centers from unit ', i3, ' in processor ',i8)") iunit, myrank
        abort = 1
        return
    endif
...
!    Allocates memory for geometry and basis set
    ...
    allocate(atmnam(ncen), av(0:2*mxn+4*mxl), bv(0:2*mxn+4*mxl), ll(ncaps), lmaxc(ncen), nf(ncaps), &
            ngini(ncen), ngfin(ncen), nn(ncaps), nzn(ncen), nshells(ncen), nbasesac(0:ncen), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error ', i5, ' in readsgbsden when allocating atmnam, av, bv, ll, lmaxc, nf, ngini, &
            & ngfin, nn, nzn, nshells, nbasesac in processor ', i8)") ierr, myrank
        abort = 1
        return
    endif
...
!    Reads geometry
    ncaps = 0
    do ia = 1, ncen
        read(iunit, *, iostat = ierr) rcen(1,ia), rcen(2,ia), rcen(3,ia), zn(ia), nshells(ia)
        if (ierr .ne. 0) then
            write(6,"('Error reading geometry from file ', a, ' in processor ',i8)") trim(projectname)//".sgbsden", myrank
            abort = 1
            return
        endif
        if (abs(zn(ia)-re(int(zn(ia) + umbrzn))) .gt. umbrzn) then
...
!    Reads basis set
    ...
    do ia = 1, ncen
        if (nshells(ia) .gt. 0) then
            ...
            do k = ngini(ia), ngfin(ia)
                i = i + 1
                read(iunit, *, iostat = ierr) nn(i), ll(i), xx(i)
                if (ierr .ne. 0) then
                    write(6,"('Error reading basis set from file ', a, ' in processor ',i8)") trim(projectname)//".sgbsden",myrank
                    abort = 1
                    return
                endif
...
!    Writes geometry and basis set to a file ".sgbs"
    ...
    i = 0
    do ia = 1, ncen
        write(15,iostat = ierr) ngini(ia), ngfin(ia)
        if (ierr .ne. 0) then
            write(6,"('Error writing basis set to file ', a, ' in processor ',i8)") trim(projectname)//".sgbs", myrank
            abort = 1
            return
        endif
        if (ngini(ia) .le. 0) cycle
        do k = ngini(ia), ngfin(ia)
            i = i + 1
            write(15,iostat = ierr) nf(i), nn(i), ll(i), xx(i), rnor(i)
            if (ierr .ne. 0) then
                write(6,"('Error writing basis set to file ', a, ' in processor ',i8)") trim(projectname)//".sgbs", myrank
                abort = 1
                return
        ...
        enddo
    enddo
!    orientation of the original axis system in the final system (coincident: for compatibility with the old codes)
    write(15,iostat = ierr) uno, cero, cero
    write(15,iostat = ierr) cero, uno, cero
    write(15,iostat = ierr) cero, cero, uno

    if (ierr .ne. 0) then
        write(6,"('Error writing orientation to file ', a, ' in processor ',i8)") trim(projectname)//".sgbs", myrank
        abort = 1
        return
    endif
...
!    Geometry and nuclear charges
    do ia = 1, ncen
        write(15,iostat = ierr) rcen(1,ia), rcen(2,ia), rcen(3,ia), zn(ia)
        if (ierr .ne. 0) then
            write(6,"('Error writing basis set to file ', a, ' in processor ',i8)") trim(projectname)//".sgbs", myrank
            abort = 1
            return
...
!    Marks the blocks of negligible charge distributions the matrix in array lsdisf
    if (ioptaj .ne. 2) then
        allocate(lsdisf(ncaps,ncaps), stat = ierr)
        if (ierr .ne. 0) then
            write(6,"('Memory error ', i5, ' in readsgbsden when allocating lsdisf in processor ', i8)") ierr, myrank
            abort = 1
            return
        endif
        do ia = 1, ncen
            ...
            enddo
        enddo
        write(15,iostat = ierr) ((lsdisf(i,j), j = 1, ncaps), i = 1, ncaps)
        if (ierr .ne. 0) then
            write(6,"('Error writing lsdisf to file ', a, ' in processor ',i8)") trim(projectname)//".sgbs", myrank
            abort = 1
            return
        endif
        deallocate (lsdisf)
    ...
    close(15)
    deallocate(atmnam, av, bv, ll, lmaxc, nbasesac, nf, ngini, ngfin, nn, nzn, nshells, rcen, rnor, xx, zn, stat = ierr)

!    Allocates the array containing the density matrix

    allocate(dmat(nbas,nbas), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating dmat in processor ',i8)") myrank
        abort = 1
        return
    endif
...
!    Reads the density matrix in lower triangle form:  read ((dmat(i,j), j = 1, i), i = 1, nbasis)
    read(iunit,*, iostat = ierr) ((dmat(i,j),j=1,i),i=1,nbasis)
    if (ierr .ne. 0) then
        write(6,"('Error reading density matrix from file ', a, ' in processor ',i8)") trim(projectname)//".sgbsden", myrank
        abort = 1
        return
    endif
    do i = 2, nbasis
...
!    Writes the density matrix to a file .den
    ...
    write(16, iostat = ierr) nbasis, ((dmat(i,j), i=1,nbasis), j=1,nbasis)
    if (ierr .ne. 0) then
        write(6,"('Error writing density matrix to file ', a, ' in processor ',i8)") trim(projectname)//".den", myrank
        abort = 1
        return
    endif
    close(16)
...
!        otherwise:     fits only the two-center terms of the density
  subroutine ajusta
    ...
    integer(KINT) :: k, kj, kml, knt, kntblock, kntden, knticf, kntlm
    integer(KINT) :: l, la, laux, lb, lenpol, lk, lm, lma, lmax, lmb, lmin, lrotar, m, ma, maxltot, mb
    integer(KINT) :: n, na, nb, nfa, nfb, nga1, nga2, ngb1, ngb2, ngb21, ngb22, nelemden
    ...
    real(KREAL), allocatable :: denvec(:), faux(:), fbux(:), fk0(:), r2l2(:), r2v(:)
...
!    Allocates memory for arrays tchvec0, tchvec1, tchvec0a, xcheb, fkvec0
    ...
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating  expvec, expvinv, f0, faux, fbux, r2l2, r2v, residuals, rpntaj, &
    ...
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating qlmdst in processor ',i8)") myrank
        abort = 1
        return
    endif
    allocate(qlmasint(lmtop), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating qlmasint in processor ',i8)") myrank
        abort = 1
        return
    endif
                        ...
                        doma: do ma = -la, la
                            ...
                            do j = i1l1l2(lma,lmb), i2l1l2(lma,lmb)
                                ...
                                rmultipfr(lml1l2(j),ia-istart+1) = rmultipfr(lml1l2(j),ia-istart+1) &
                                         + bux * qlm1c(llm(lml1l2(j))) * dosl1i(llm(lml1l2(j)))
...
!    TWO-CENTER CONTRIBUTIONS
        if (ioptaj .ne. 2) then        ! In case ioptaj .eq. 2, only takes one-center distributions
            lskip = .false.
            do ib = 1, ncen                  ! Do over centers (ib)
!                if (lskip(ib) .or. ia .eq. ib .or. ngini(ib) .le. 0) cycle
                kntblock = kntblock + 1
                if (lskip(ib) .or. .not. lblockproc(kntblock)) cycle    	! skips to next center ib! skips to next center ib
                xb = rcen(1,ib)
                yb = rcen(2,ib)
                zb = rcen(3,ib)
!    Computes Euler angles and rotation matrices from the AB aligned axis system to the molecular system and conversely
                ...
                if (rab .lt. 1.d-10) then
                    write(6,"('Error in proc ', i8, ' Centers ',i5,' and ',i5,' coincide')") myrank, ia, ib
                    abort = 1
                    return
                endif
                ngb1 = ngini(ib)
...
!    Computes Euler angles and rotation matrices from the AB aligned axis system to the molecular system and conversely
                    ...
                    do i1 = nga1, nga2
                        ...
                        do i2 = ngb21, ngb22
                            knt = knt + 1
                            if (.not. lsdisf(i1,i2)) then
!                                kntden = kntden + (2*la+1) * (2*lb+1)
                                cycle    ! If the distribution is negligible, skips to next
                            endif
                            nb = nn(i2)
                            lb = ll(i2)
                            exb = xx(i2)
...
!    Checks whether the radial factor is negligible or not. If it is, marks it and avoids fitting.
                    ...
                    if (fmax .lt. umbral) then    ! Test: is the fragment negligible ?
                        ...
                        do j = 2, mxlenpol-1
                            ...
                            res = cux * cux * dot_product( ( faux - fkvec0 * fbux), &
                                    (faux - fkvec0 * fbux))
                        ...
                        enddo
                        if (lenpol .eq. 0) then
                            if (longoutput) write(6,"(/'WARNING: Residual in interval ', i3, ' of radial factor with l =',&
                                    i3, ' m = ', i3, ' of center ',i8, ' = ', e12.5,/)") i, l, m, ia, res
                            lenpol = mxlenpol
                        endif
...
!    fitting coeficients of auxiliary integrals for electrostatic potential and field
    ...
    deallocate(dl, dosxcheb, expvec, expvinv, f0, fa, faux, fk0, ftab, fkvec0, ha, qlm2c, qlmasint, qlmdst, r2pow, &
            rintr1, rintr2l2, rl, rlt, rpntaj, rpow, tchvec0, tchvec1, tchvec0a, vaux1c, xcheb, ymat1, ymat2)
    return
    end
!
!    ***************************************************************
...
!
  subroutine frgsigma(knt, n, np, lmax, za, zb, Rg)
    USE DAM_400_D
    USE DAM_400_CONST_D, ONLY : cero, uno, dos, umed, pi, re, ri, fact, facti
    USE DAM_400_DATA_D, rv => rpntaj
    implicit none
            ...
            do ma = -la, la
                ...
                dol2: do l = abs(ma), lbexp
                    ...
                    do j = i1l1l2(lma,kmb), i2l1l2(lma,kmb)
                        ...
                        fa(1:npntaj,lml1l2(j)) = fa(1:npntaj,lml1l2(j)) + aux * r2pow(1:npntaj,npl1l2(j)) &
                                * ha(1:npntaj,lma)
                    enddo
                enddo dol2
            enddo
...
!
  subroutine Avk(xa, xb, r, Rab, a00, am10, a0m1, am1m1)
    USE DAM_400_D
    USE DAM_400_DATA_D , ONLY : wthreshold
    USE DAM_400_CONST_D
    implicit none
...
!    Calculation of Kint(m,w,w1) for m = 0  and (m = -1
    ...
    if ((w-w1) .lt. 1.d-5) then
        ...
        do j = 1, jmax
            sgn = -sgn
            rKint0 = rKint0 - sgn * bi(j) * bk(j) * dosl1i(j-1) * dosl1i(j)
            rKintm1 = rKintm1 + re(j+1) * re(j+1) * h3f2(j) * bi(j+1) * bk(j+1)
        enddo
        rKint0 = cuatro * rKint0
        rKintm1 = rKintm1 * r16 / (w*w - w1*w1)
        ...
        if (w .gt. wthreshold) then
            ...
            if (w .lt. 25d0) then
                Gint0 = G00(w)
                Gintm1 = bkm1 - Gint0    ! Recursion A3.10
                x = w1*w1 / wsq
                ai = uno
                s0 = Gint0
                sm1 = Gintm1
                do i = 1, 30
                    Gint0 = (wsq * Gint0 + dosl1(i-1) * bk0 - bk1) * dosl1i(i-1) * dosl1i(i-1)        ! Recursion A3.11
                    Gintm1 = (wsq * Gintm1 + dosl1(i-1) * bkm1 - bk0) * dosl1i(i-1) * dosl1i(i)    ! Recursion A3.11
                ...
                g0(30) = G030(w)
                do i = 29, 0, -1
                    g0(i) = wsqi * (dosl1(i) * dosl1(i) * g0(i+1) - dosl1(i) * bk0 + bk1)        ! Recursion A3.11 (downwards)
                enddo
                x = w1*w1 / wsq
                ai = uno
                s0 = g0(0)
                sm1 = bkm1 - g0(0)    ! Recursion A3.10
                do i = 1, 30
                    ai = ai * (re(i)-umed) * x * ri(i)
                    s0 = s0 + ai * g0(i)
                    sm1 = sm1 + ai * (bkm1 - g0(i)) * dosl1i(i)
                enddo
            endif
        endif
        aux = dos / sqrt(w*w - w1*w1)
        rKint0 = aux * (rKint0 - w * s0)
        rKintm1 = aux * (rKintm1 - w * sm1)
    endif
    if (w*w .le. dos * w2*w2) then    !    Case of w <= sqrt(2) w2 : series of products of Bessel I and K functions (30 terms)
        ...
        do j = 1, jmax
            sgn = -sgn
            rKcomp0 = rKcomp0 - sgn * bi(j) * bk(j) * dosl1i(j-1) * dosl1i(j)
            rKcompm1 = rKcompm1 + re(j+1) * re(j+1) * h3f2(j) * bi(j+1) * bk(j+1)
        enddo
        rk0w2 = exp(-w2) * pi / sqrt(w*w - w2*w2)    !    rk0w2 = sqrt(2*pi/(w^2-w2^2)) * khat(1/2,w2)
        rKcomp0 = rk0w2 - cuatro * rKcomp0
        rKcompm1 = rk0w2 / w2 - rKcompm1 * r16 / (w*w - w2*w2)
    ...
    a00 = (rk0w1 + umed * (xa * sqrR - xb * r) * rKint0 - umed * (xa * sqrR + xb * r) * rKcomp0) / (dos * xa * sqrR)
    am10 = (rkm1w1 + umed * (xa * sqrR - xb * r) * rKintm1 - umed * (xa * sqrR + xb * r) * rKcompm1) / (dos * xa * sqrR)
    a0m1 = (rk0w1 + umed * (xa * sqrR - xb * r) * rKint0 + umed * (xa * sqrR + xb * r) * rKcomp0) / (dos * xb * r)
    am1m1 = (rkm1w1 + umed * (xa * sqrR - xb * r) * rKintm1 + umed * (xa * sqrR + xb * r) * rKcompm1) / (dos * xb * r)
    return
    end
!
...
!
  function G00(x)
    USE DAM_400_D
    USE DAM_400_CONST_D, ONLY : cero, uno, umed, pt25, pimed, euler
    implicit none
    real(KREAL) :: G00, x, y
    if (x .lt. 10.d0) then
...
!
  subroutine BesselK01(x, bk0, bk1)
    USE DAM_400_D
    USE DAM_400_CONST_D, ONLY : uno, umed
    implicit none
...
!
  subroutine BesselIK1011(x, xk, bi, bk)
    USE DAM_400_D
    USE DAM_400_CONST_D, ONLY : uno, umed, re
    implicit none
...
!
  subroutine bvk0(xa, xb, xa2, xb2, Rabsq, r, rsq, bm1, b0)
    USE DAM_400_D
    USE DAM_400_CONST_D, ONLY: cero, uno, dos, umed, pi
    USE DAM_400_LEGENDRE_D
    implicit none
...
!
  subroutine multipan(na, la, nb, lb, lmin, lmax, za, zb, R)
    ...
    USE PARALELO
    implicit none
    integer(KINT) :: i, j, k, kk, kma, kmb, kp, kq, l, la, lb, lmax, lmin, m, ma, mb, na, nb
    real(KREAL) :: argk1, argk1sq, argk1sqi, argk2, argk2sq, argk2sqi, aux, auxi, bux, cux, dux, R, sgn, thrasint, za, zb
...
!     Computes the multipolar moments by numerical integration (Legendre quadrature)
  subroutine multipquad(na, la, nb, lb, lmin, lmax, za, zb, R)
    ...
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating: bkmat, powu and pow1mu.')")
        abort = 1
        return
    endif
    powR(0) = uno
...
!    Angular normalizacion coefficients: ang(l*(l+1)/2+m+1) = sqrt( (2*l+1) * fact(l-m) / (2 * pi * (1 + delta(m,0)) * fact(l+m)) )
    ang(1) = umed / raizpi
    lm = 1
    do l = 1, lmaxexp
        lm = lm + 1
...
!    Constants for overlap integrals with ellipsoidal coordinates
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error when allocating ap, alfasol, ipntap, ipntalfa.')")
        abort = 1
        return
    endif
    if (myrank .eq. 0 .and. longoutput) then
        write(6,"('Size of ap   = ', i15, ' bytes')") size(ap)
        write(6,"('Size of alfasol   = ', i15, ' bytes')") size(alfasol)
        write(6,"('Size of ipntap   = ', i15, ' bytes')") size(ipntap)
        write(6,"('Size of ipntalfa   = ', i15, ' bytes')") size(ipntalfa)
    endif
...
!
  subroutine acof
    USE DAM_400_D
    USE DAM_400_DATA_D, only: mxltot
    USE DAM_400_CONST_D
    implicit none
...
!
  subroutine totalchargeSTO
    ...
    allocate(sol(0:mxl), rl(-mxl:mxl,-mxl:mxl,0:mxl), dl(-mxl:mxl,-mxl:mxl,0:mxl), &
            stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating sol, rl, dl.')")
        abort = 1
        return
    endif
    charge = cero
...
!    Computes Euler angles and rotation matrices from the AB aligned axis system to the molecular system and conversely
                    ...
                    rab = sqrt(xab*xab + yab*yab + zab*zab)
                    if (rab .lt. 1.d-10) then
                        write(6,"('Centers ',i5,' and ',i5,' coincide')") ia, ib
                        abort = 1
                        return
                    endif
                    if (xy .gt. 1.d-10) then
...
!    Rotation on center A
                    ...
                    do m = -min(la,lb), min(la,lb)
                         charge = charge + rna * rnb * ang(ind(la)+abs(m)+1) * ang(ind(lb)+abs(m)+1) * roblk(m,m) * sol(abs(m))
                    enddo
                enddo
            enddo
        enddo
...
!
  subroutine abintegs(na, la, za, nb, lb, zb, R)
    ...
    USE PARALELO
    implicit none
    integer(KINT) :: i, ierr, imax, j, jlim, jmax, la, lb, na, nb
...
!
    ...
    if (gamma .eq. cero) then
        ...
        if (jmax .gt. jlim) then
            ...
            enddo
            suma = suma + si * ( (re(jmax+2+i+i)+gamma) * expgi + (re(jmax+2+i+i)-gamma) * expg )
            write(6,"('The series for the integral B(',i5,',',e22.15,') did not converge with the prescribed accuracy after ', &
                    i5,' terms')") jmax, gamma, imax
