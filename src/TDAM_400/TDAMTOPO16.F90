...
!       open(iout,file=trim(filename)//"-DAMTOPO_400.out",status="replace")  ! Sealed by Rafa
...
!        close(125)
!        close(iout)
!        call system ("cat "//trim(filename)//".pv >> "//trim(filename)//"-DAMTOPO_400.out")
...
! ---------------     End of change made by Rafa   -------------- 
      ...
      write(iout,"(a)")"Copyright 2008-2026, Jaime Fernandez Rico, Rafael Lopez, Ignacio Ema,"
