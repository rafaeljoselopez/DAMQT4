!  Copyright 2011-2026, Jaime Fernandez Rico, Rafael Lopez, Ignacio Ema,
!  Guillermo Ramirez
! 
...
!
! Requires file DAM_400_GLOBAL.F90 which must be compiled first (contains the modules)
!
...
! #define DBLPRCGRID    ! Uncomment this line  if double precision grid is wanted
  program DAMDENZernike310
    ...
    namelist / options / dltu, dltv, dltx, dlty, dltz, filename, fileZJname, indices, iswindows, kmaxrep, lechelon, lgradient, &
            ...
            numrtab, planeA, planeB, planeC, planecase, rtab, uinf, usup, vinf, vsup, x_func_uv , xinf, xsup, &
            y_func_uv, yinf, ysup, z_func_uv, zinf, zsup
...
!    endif
    ...
    write(6,"(/3x,'Using expansion in file: ', a)") trim(fileZJname)
...
!    Opens file for electrostatic potential tabulation
    ...
    iuni = 21
    if (ljacobi) then
        tipo = "-jacobi_"//trim(planesuffix)
    else
        tipo = "-zernike_"//trim(planesuffix)
    endif
