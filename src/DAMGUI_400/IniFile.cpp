...
//------------------------------------------------------------------------
//
//	Class for reading and writing *.ini files. These are text files
//	that can be used both in windows and Linux
//	Structure:
//		[section]
//		variable=value
//	File .ini is asked for section and variable and returns the value
//
//  File:   IniFile.cpp
//
...
{
//    qDebug() << "en SetValue: KeyName = " << KeyName.c_str() << " Value = " << Value.c_str() << " SectionName = " << SectionName.c_str() ;
	vector<Record> content;     // Holds the current record
	if (Load(FileName, content))    // Make sure the file is loaded
	{
