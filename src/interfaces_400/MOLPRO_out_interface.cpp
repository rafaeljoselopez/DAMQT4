...
void readbasisset(ifstream * inputfile, ofstream * outimportfile, ofstream * ggbsfile, int * indbases, string MOLPROfiles){
    string oldrepirred, repirred, s, ss, ss_trimmed;
...
/*    seek for the basis set */
        if( !(s.find("BASIS DATA")==string::npos) ) {
            ...
            ss_trimmed = ss;
            if ((rtrim(ss_trimmed).length() < posbasis[4]) || !(ss.substr(posbasis[4],posbasis[5]-posbasis[4]).find(".")==string::npos)){  // more than one line for contraction coefficients
                skipgetline = true;
            }
            else{
                skipgetline = false;
            ...
            while (!lbasis){
                ...
                for (j = 0 ; j < nfn-1 ; j++){
                    ...
                    kntline++;
                    ss_trimmed = ss;
                    if ((rtrim(ss_trimmed).length() < posbasis[4]) || !(ss.substr(posbasis[4],posbasis[5]-posbasis[4]).find(".")==string::npos)){
                        skipgetline = true;
                    }
                    else{
                        skipgetline = false;
