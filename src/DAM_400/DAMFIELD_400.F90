...
!	Namelist default values
    ...
    lextralines = .false.	! If .true. reads extra lines from namelist or from a file
    ...
    filename = ""			! root file name for .field2(3)D files
...
!	Opens file for field lines
    if (lplot2d) then
        open(12,file=trim(filename)//".field2D",status='unknown',form='formatted',iostat=ioerr)
        if (ioerr .ne. 0) call error(1,'Error when opening file '//trim(filename)//".field2D"//'.Stop')
    else
        open(12,file=trim(filename)//".field3D",status='unknown',form='formatted',iostat=ioerr)
        if (ioerr .ne. 0) call error(1,'Error when opening file '//trim(filename)//".field3D"//'.Stop')
    endif
!    Force lines
    npntot = 0
    nlineas = 0
...
!write(6,"('ex = ', e12.5, ' ey = ', e12.5, ' ez = ', e12.5, ' u = ', e12.5, &
!' v = ', e12.5, ' gu = ', e12.5, ' gv = ', e12.5,///)") &
!ex, ey, ez, u, v, gu, gv
...
!write(6,"('ex = ', e12.5, ' ey = ', e12.5, ' ez = ', e12.5, ' u = ', e12.5, &
!' v = ', e12.5, ' gu = ', e12.5, ' gv = ', e12.5,///)") &
!exfor, eyfor, ezfor, ufor, vfor, gufor, gvfor
        endif
!		Updates the point
        u = ufor
        v = vfor
...
!                    Ex = -umed * pi4d2l1 * c1 * rinta * ra2l3inv
...
!                    Ey = -isgm * umed * pi4d2l1 * c1 * rinta * ra2l3inv
...
!                    Ez = -pi4d2l1 * c1 * rinta * ra2l3inv
    ...
    Ey = Eyext + Eynucl
    Ez = Ezext + Eznucl
!write(6,"('campo: ia = ', i3, ' x = ', e12.5, ' y = ', e12.5, ' z = ', e12.5, ' Ex = ', e12.5, &
!    ' Ey = ', e12.5, ' Ez = ', e12.5)") ia, x, y, z, Ex, Ey, Ez
!write(6,"('E nucleo: Exnucleo = ', e12.5, ' Eynucleo = ', e12.5, ' Eznucleo = ', &
!    e12.5)") Exnucl, Eynucl, Eznucl
!write(6,"('E electrones: Exext = ', e12.5, ' Eyext = ', e12.5, ' Ezext = ', &
!    e12.5)") Exext, Eyext, Ezext
    return
    end
...
!    Long-range Electric field in point (x,y,z)
    ...
    do ia = 1, ncen
        ...
        ra2inv = uno / ra2
        if (ra .lt. rlargo(ia)) then
!write(6,"('En el punto x = ', e12.5, ' y = ', e12.5, ' z = ', e12.5, &
!' para ia = ', i3, ' ra = ', e12.5, ' rlargo(ia) = ', e12.5)") &
!x, y, z, ia, ra, rlargo(ia)
!            interv = indintrv(int(fct*ra)+1)
!            ltop = lcorto(interv,ia)
            lnucleo = .true.  ! A trick to prevent evaluation outside the long-range region
            return
        else
            ltop = llargo(min(int(ra),mxlargo),ia)
        endif
...
!                Ez = -rmultip(lm,ia)  * c1
    ...
    Ey = Eyext + Eynucl
    Ez = Ezext + Eznucl
!write(6,"('campolargo: ia = ', i3, ' x = ', e12.5, ' y = ', e12.5, ' z = ', e12.5, ' Ex = ', e12.5, &
!    ' Ey = ', e12.5, ' Ez = ', e12.5)") ia, x, y, z, Ex, Ey, Ez
!write(6,"('E nucleo: Exnucleo = ', e12.5, ' Eynucleo = ', e12.5, ' Eznucleo = ', &
!    e12.5)") Exnucl, Eynucl, Eznucl
!write(6,"('E electrones: Exext = ', e12.5, ' Eyext = ', e12.5, ' Ezext = ', &
