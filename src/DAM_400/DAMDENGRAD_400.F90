...
!     Namelist default values
    ...
    filename = ""               ! root file name for .dengr2D(3D) files
...
!     Opens file for density gradient lines
    if (lplot2d) then
        ...
        open(12,file=trim(filename)//".dengr3D",status='unknown',form='formatted',iostat=ioerr)
        if (ioerr .ne. 0) call error(1,'Error when opening file '//trim(filename)//".dengr3D"//'.Stop')
    endif
