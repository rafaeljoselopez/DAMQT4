!  Copyright 2011-2026, Rafael Lopez, Gabriel Urquiza Carvalho
! 
!  This file is part of Zernike_Jacobi_400 package.
!  Zernike_Jacobi_400 is distributed in the hope that it will be useful,
!  but WITHOUT ANY WARRANTY; without even the implied warranty of
!  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
!  GNU General Public License for more details.
! 
!  You should have received a copy of the GNU General Public License
!  along with Zernike_Jacobi_400.  If not, see <http://www.gnu.org/licenses/>.
!
!------------------------------------------------------------------------
! 
...
!
  program DAMZernike_Jacobi_GAUSS_MPI_400
    USE MPI
    USE Zernike_Jacobi_GTO_MPI_D
    USE PARALELO
    implicit none
    ...
    namelist / options / iswindows, lechelon, ljacobi, longoutput, lexpansion, lrstarrel, lvalence, lzdo, &
...
!     nquadpoints = min(120,nquadpoints)
        ...
        lnamelist = (/ lechelon, ljacobi, longoutput, lrstarrel, iswindows, lvalence, lzdo /)
    endif
    CALL MPI_BCAST(projectname,len(projectname),MPI_CHARACTER,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(lnamelist,7,MPI_LOGICAL,0,MPI_COMM_WORLD,ierr)
    ...
    if (ierr .ne. 0) then
        write(6,"('Memory error when allocating inamelist and rnamelist in processor ',i3)") myrank
        abort = 1
    endif
    ...
    if (myrank .eq. 0)    then
        inamelist(:) = (/ lexpansion, kexpansion, nquadpoints /)
    ...
    if (myrank .ne. 0) then
        lechelon = lnamelist(1); ljacobi = lnamelist(2); longoutput = lnamelist(3); lrstarrel = lnamelist(4);
        iswindows = lnamelist(5); lvalence = lnamelist(6) ; lzdo = lnamelist(7)
        lexpansion = inamelist(1); kexpansion = inamelist(2); nquadpoints = inamelist(3)
...
!    Reads geometry, basis set and density matrix generated by an external program 
    ...
    if (myrank .eq. 0) then
        if (ljacobi) then
            write(6,"(/'**** Jacobi expansion ****',/)")
        else
            write(6,"(/'**** Zernike expansion ****',/)")
        endif
...
!
  subroutine leedatgauss
    ...
    USE PARALELO
    implicit none
    integer(KINT) :: i, ia, ib, icarga, ierr, indnf, indng, ios, j, k, k1, k2, knt, nbasis, ncapserr
...
!    Reads the number of centers
    ...
    if (ierr .ne. 0) then
        write(6,"('Error ', i5, ' Cannot open file ', a, ' in processor ',i3)") ierr, trim(projectname)//".ggbs", myrank
        abort = 1
        return
    endif
    read(15,*) ncen
...
!    Allocates memory for geometry and basis set
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating cfcontr0 in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating ipntprim in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating isort in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating ncontr in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating nprimit in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating xia and xib in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating xxg0 in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    do ia = 1, ncen
        ...
        do j = 1, ncontr(ia)
            ...
            if (icarga+nprimit(ncaps) .gt. mxcap*mxprimit)  then
                write(6,"('Error: maximum number of total primitives exceeded in processor ',i3)") myrank
                abort = 1
                return
            endif
...
!            actualizes the index for loading
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating xxg in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating cfcontr in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (lmaxbase .gt. mxl) then
        write(6,"('Error: highest value of l in basis set exceeded in processor ',i3)") myrank
        abort = 1
        return
    endif
...
! 
  subroutine expand(clmn_subr, frad_fun)
    ...
    integer(KINT) :: ishift, j, k, klmi, kml, knt, kntdst, kntdstef, kntdsteftot, kntdsttot, knticf, kntlm, kntproc
...
!    Allocates memory for arrays dl, fa, fcent, flm, flmmamb, ftot, rl, rlt
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating rquad01, rquadscal and rquadaux in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating weights01, weights and vaux in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
...
!     Computes molecular multipoles from Zernike 3D or Jacobi expansions
        ...
        tiemponw = dtime(tarraynw)
        write(6,"(1x,//80('='),/'IMPORTANT !!! Projection on Canterakis-Zernike or Jacobi functions is not parallelized',/)")
        write(6,"(1x,'Timing in seconds for projection (user, system, total):', &
                /5x,'(',e12.5,',',e12.5,',',e12.5,')')") tarraynw(1), tarraynw(2), tarraynw(1)+tarraynw(2)
...
!     Writes Zernike 3D or Jacobi moments to .zernike or .jacobi file
        ...
        if (ljacobi) then
            ...
            if (ierr .ne. 0) then
                write(6,"('Error ', i5, ' Cannot open file ', a, ' in processor ',i3)") ierr, trim(projectname)//".jacobi", myrank
            endif
        else
            open(99,file=trim(projectname)//".zernike",form='formatted', iostat=ierr)
            if (ierr .ne. 0) then
                write(6,"('Error ', i5, ' Cannot open file ', a, ' in processor ',i3)") ierr,trim(projectname)//".zernike", myrank
            endif
        endif
...
!     into functions of the same type
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating ssv, sdv, msv and mdv in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    do m2 = -mxemes, mxemes
...
!    regular spherical harmonics
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating app and bpp in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    call acof
...
!    Tabulates some auxiliary indices for locating the previous coefficients
    ...
    if (ierr .ne. 0) then
        write(6,"('Memory error', i5, ' when allocating indk12 in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    do l2 = 0,mxltot
...
!    Polynomials P_k^(L,M;L',M')(0,0,1) of the shift-operators technique in the alligned-axes system
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating ipntpolP in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    ...
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating polP in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
...
!
   subroutine fradABgauss(i1, i2, za, xb, zb)
    USE Zernike_Jacobi_GTO_MPI_D, lmxc => lexpansion, rflm => rquadscal
    implicit none
    ...
    real(kreal) :: rac, rbc, rinf, rmax, rmin, sinb, xb, xxa, xxabni, xxasq, xxb, xxbsq, xiij, xij, za, zb, zij
...
! 
  subroutine recurflm(ldim, lmaxtot, la, lb, r2, za, xb, zb, fin)
    USE Zernike_Jacobi_GTO_MPI_D, fvoid => flm
    USE PARALELO
    implicit none
    logical :: lcux
...
!
  subroutine totalchargeGTO
    ...
    allocate(sol(0:mxl), rl(-mxl:mxl,-mxl:mxl,0:mxl), dl(-mxl:mxl,-mxl:mxl,0:mxl), stat = ierr)
    if (ierr .ne. 0)  then
        write(6,"('Memory error', i5, ' when allocating sol, rl and dl in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    charge = cero
