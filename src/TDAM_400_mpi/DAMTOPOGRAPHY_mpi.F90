!  Copyright 2013-2026, Jaime Fernandez Rico, Rafael Lopez, Ignacio Ema,
!  Guillermo Ramirez, Anmol Kumar, Sachin D. Yeole, Shridhar R. Gadre
! 
!  This file is part of DAM_400.
! 
!  DAM_400 is free software: you can redistribute it and/or modify
!  it under the terms of the GNU General Public License as published by
!  the Free Software Foundation, either version 3 of the License, or
!  (at your option) any later version.
! 
!  DAM_400 is distributed in the hope that it will be useful,
!  but WITHOUT ANY WARRANTY; without even the implied warranty of
!  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
!  GNU General Public License for more details.
! 
!  You should have received a copy of the GNU General Public License
!  along with DAM_400.  If not, see <http://www.gnu.org/licenses/>.
!
!> @file TDAMTOPO15_mpi.F90
...
!> \a e. Invokes MPI for parallelization.
!>
...
!> @param [in] addguess Enables user to provide additional guess points.
  ...
  Program Topodriver
    ...
    namelist / options / med, mesp, topograph,gradpath, &
            ...
            & boxl,boxg,boxb,boxt,stepszt,angle,exln, &
            & lmaxi,filename,guessfile,iswindows,fdisp, drcutcp, ncntguess, rcntguess
    CALL MPI_INIT(ierr)
...
!-------------------------------------------------------------------------------------
    ...
    if (myrank.eq.0) then
! 		Reads the options defined by user and the projectname 
! ----------      change made by Rafa          -----------------------------
...
!    	open (iout, file = trim(filename)//"-DAMTOPO_400_mpi.out", status="replace") ! Sealed by Rafa
        inamelist = (/ lmaxi /)
        rnamelist = (/ cnvg, boxl, boxg, boxb, boxt, stepszt, angle, exln, fdisp /)
        allocate(timeprocs(2*nprocs), stat = ierr)
        if (ierr .eq. 0) then
            ltimeprocs = .true.
            timeprocs = 0.
        else
            write(6,"('WARNING: Error num ', i5, ' in TOPODRIVER when allocating timeprocs')") ierr
            ltimeprocs = .false.
        endif
        lnamelist = (/ med,mesp,topograph,gradpath,basin, &
                & lgradient, lderiv2, largo, lexact, wireframe, &
                & solidsurf, addguess, exdraw, iswindows, ltimeprocs, lvalence /)
    end if
    ...
    CALL MPI_BCAST(guessfile,len(guessfile),MPI_CHARACTER,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(lnamelist,16,MPI_LOGICAL,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(inamelist,1,MPI_INTEGER4,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(rnamelist,9,MPI_REAL8,0,MPI_COMM_WORLD,ierr)

    if (myrank.ne.0) then
        lmaxi = inamelist(1)
        med=lnamelist(1); mesp=lnamelist(2) !; topomanual = lnamelist(3)
        topograph = lnamelist(3); gradpath = lnamelist(4)
        basin=lnamelist(5); lgradient=lnamelist(6); lderiv2=lnamelist(7)
        largo=lnamelist(8); lexact=lnamelist(9); wireframe=lnamelist(10)
        solidsurf=lnamelist(11); addguess=lnamelist(12)
        exdraw = lnamelist(13); iswindows=lnamelist(14); ltimeprocs = lnamelist(15)
        lvalence = lnamelist(16)
        cnvg=rnamelist(1); boxl=rnamelist(2); boxg=rnamelist(3)
        boxb=rnamelist(4); boxt=rnamelist(5); stepszt=rnamelist(6)
        angle=rnamelist(7); exln = rnamelist(8); fdisp = rnamelist(9)
    end if
...
!    if (basin) solidsurf=.true.     ! modified by Rafa 2019
    ...
    if (myrank.eq.0) then
                    ...
                    write(iout,"('number of processors = ', i3)") nprocs
    end if

    CALL MPI_BARRIER(MPI_COMM_WORLD,ierr)
    if (med) call predamden_mpi
    if (mesp) call predampot_mpi
    CALL MPI_BARRIER(MPI_COMM_WORLD,ierr)
    ...
    if (myrank.eq.0) then
        ...
        write(iout,"(80('-'))")
            do i = 1 , ncen
                write(iout,"(a2,i3,t27,f12.6,t44,f12.6,t64,f12.6)") atmnam(i),i,rcen(1,i),rcen(2,i),rcen(3,i)
            end do
        write(iout,"(80('-'))")
        write(iout,"(' ')")
    end if
...
!======Center of mass====================
    ...
    if (ltimeprocs) then
        ...
        if (ierr .eq. 0 .and. myrank .eq. 0) then
            write(6,"(/30x,'New TIMING (in seconds)',/)")
            do i = 0, nprocs-1
...
!******************************************      
    ...
    write(irankd,"(i4.2)") myrank
    allocate(istav(0:nprocs), iendv(0:nprocs-1), stat = ierr)
    if (ierr .ne. 0) then
        write(iout,"('Error num ', i5, ' in TOPO when allocating istav and iendv in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif

    if (myrank.eq.0.and.med) then
        write(iout,"(a,I3)") "Euler Characteristic = -1"
    end if
!      if (mesp) then   ! FOLLOWING IS TO FIND OUT THE EULER
                        ! CHARACTERISTICS OF MESP OF A MOLECULE
                        ! AND CREATE GUESS POINTS

    open ( unit = 31, file = trim(filename)//".ts_"//trim(adjustl(irankd)),access="append", status="replace")
...
! 		Sr is the radius thus found for the sphere on which Euler characteristic is to be determined.
        CALL findeuler(Sr)
    end if
!	++++++++++++++++++++++++++++++
    bx = (xmax) - (xmin)
    by = (ymax) - (ymin)
    bz = (zmax) - (zmin)
! 	Bx, By and Bz defines the box to be used for guess point generation
...
!	and guess point produced are very close to actual CP
    ...
    nstepz = bz/stepszt
    if (myrank.eq.0) then
        write(iout,"(a)") "Guess points are created using &
    ...
    call para_range(nstepz)
! 	Following if statement ensures overlap between the grid chunk
! 	distributed over different processors so that guess points at
! 	the boundaries of chunks are not missed
    if (nprocs.ne.1) then
        if (myrank.eq.0) then
            istav(myrank)=istav(myrank)
            iendv(myrank)=iendv(myrank)+2
        elseif (myrank.gt.0.and.myrank.lt.nprocs-1) then
            istav(myrank)=istav(myrank)-2
            iendv(myrank)=iendv(myrank)+2
        elseif (myrank.eq.nprocs-1) then
            istav(myrank)=istav(myrank)-2
            iendv(myrank)=iendv(myrank)
        end if
    end if
    allocate (sumder(nstepx,nstepy,istav(myrank):iendv(myrank)), fval(nstepx,nstepy,istav(myrank):iendv(myrank)), stat = ierr)
    if (ierr .ne. 0) then
        write(iout,"('Error num ', i5, ' in TOPO when allocating sumder and fval in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
...
! 	at all the grid points which fall inside the box, in an array called sumder.  
    ...
    do i = 1,nstepx
        do j = 1,nstepy
            do k = istav(myrank),iendv(myrank)
                x1 = xmin + stepszt * (i - 1)
                y1 = ymin + stepszt * (j - 1)
                z1 = zmin + stepszt * (k - 1)
                if (med) then
                    CALL DAMDEN_mpi(den,drvxtot,drvytot,drvztot,dxxtot, &
                            & dxytot,dxztot,dyytot,dyztot,dzztot,x1,y1,z1)
                    fval(i,j,k) = dabs(den)
                elseif (mesp) then
                    CALL DAMPOT_mpi(vtot,drvxtot,drvytot,drvztot, &
...
!	These serve as guess points to MESP.   
    ...
    do i = 2,nstepx-1
        do j = 2,nstepy-1
            do k = istav(myrank),iendv(myrank)
                if (k.gt.istav(myrank).and.k.lt.iendv(myrank)) then
                    if (fval(i,j,k).le.0.00005) cycle
                    c1=abs(sumder(i,j,k))
                    c2=abs(sumder(i+1,j,k))
                    c3=abs(sumder(i-1,j,k))
                    c4=abs(sumder(i,j+1,k))
                    c5=abs(sumder(i,j-1,k))
                    c6=abs(sumder(i,j,k+1))
                    c7=abs(sumder(i,j,k-1))
                    if(c1.lt.c2.and.c1.lt.c3.and.c1.lt.c4.and.c1.lt.c5.and.c1.lt.c6.and.c1.lt.c7) then
                        x1 = xmin + stepszt * (i - 1)
                        y1 = ymin + stepszt * (j - 1)
                        z1 = zmin + stepszt * (k - 1)
                        write(31,"(3(f9.4,2x),I2)")x1,y1,z1
                    endif
                end if
            enddo
        enddo
    ...
    if (myrank.eq.0) then
        ...
        if (abort .eq. 0) call aroundnucleus  ! Creates extra guess points around nuclear positions.
        if (abort .eq. 0) call sortgcps
        base = trim(filename)//".ts"
        call remove_single_file(base, ierr)
        if (addguess .and. (abort .eq. 0)) then
            ...
            if (ierr.ne.0) then
                ...
                if (len_trim(guessfile) > 0) then   ! Gess points given in file guessfile
                    ...
                    if (.not. lexist) then
                        ...
                        if (ierr.ne.0) then
                            write(6,"('Cannot open file ', a, ' with guess points. ierr = ', i5)") &
                                trim(adjustl(guessfile)), ierr
                        else
...
! 	around nucleus are not used further in the program.
    CALL MPI_BARRIER(MPI_COMM_WORLD,ierr)
    CALL OPTDRIVER
    CALL MPI_REDUCE(abort,abortroot,1,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(abortroot,1,MPI_LOGICAL,0,MPI_COMM_WORLD,ierr)
...
!***************************************************      
    ...
    REAL(KREAL) :: X1,Y1,Z1,den,vtot,drvxtot,drvytot,drvztot,dxxtot, dxytot,dxztot,dyytot,dyztot,dzztot
    real(kreal):: Sr
    ...
    do i = 1,37
        do j=1,19
            ...
            if (i.gt.1.and.j.eq.19) cycle   ! to ensure that when phai is 180 degrees
                                            ...
                                            ! theta are considered only once
            CALL DAMPOT_mpi(vtot,drvxtot,drvytot,drvztot,dxxtot,dxytot,dxztot, &
...
! 	Calculating number of negative islands 
    ...
    do ist = 1,maxn_iln
        if (nil3(ist).eq.0) then
            nn_iln=nn_iln+1
            do i = ist,k      ! loop starts from index where it finds nil3 to be zero
                if (i.eq.ist) then  ! if it is first cycle
                    do j = i+1,k      ! Run next cycle
...
! 	Calculating number of positive islands 
    ...
    do ist=1,maxp_iln
        if (pil3(ist).eq.0) then
            ...
            do i = ist , m      ! loop starts from index where it finds pil3 to be zero
                if (i.eq.ist) then  ! if it is first cycle
                    do j = i+1,m      ! Run next loop
...
! ----------      change made by Rafa          -----------------------------
    ...
    if (ierr .ne. 0) then
        write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//".ts", myrank
        abort = 1
        return
    endif
! ----------      End of change made by Rafa   ----------------------------  
...
!*****************************************     
    ...
    h=3.7
    open(32,status="SCRATCH",iostat=ierr)
    if (ierr .ne. 0) then
        write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, "SCRATCH", myrank
        abort = 1
        return
    endif
    do i=1,ncen
    ...
    allocate(xg(NCEN*14),yg(NCEN*14),zg(NCEN*14), stat = ierr)
    if (ierr .ne. 0) then
        write(iout,"('Error num ', i5, ' in AROUNDNUCLEUS when allocating xg, yg and zg in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
...
!! distance criterion.
  SUBROUTINE SORTGCPS
    ...
    enddo
123 rewind(1)
    allocate (xi(icps),yi(icps),zi(icps), stat = ierr)
    if (ierr .ne. 0) then
        write(iout,"('Error num ', i5, ' in SORTGCPS when allocating xi, yi and zi in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    do i = 1,icps
...
223 close(1)
    ...
    if (ierr .ne. 0) then
        write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//".gcp", myrank
        abort = 1
        return
    endif
...
!**********************************************      
    USE MPI
    USE DAMINITIAL_T, ONLY: CNVG, BOXL, PROJECTNAME,NCEN, RCEN,iout, &
            & MED,MESP, FILENAME, KINT,KREAL,KREAL4,KINT8
    ...
    INTEGER(KINT), DIMENSION(4)   :: ntypfr,ntyp,nstrt
    REAL(DP)                      :: T1, T2
    INTEGER(KINT)                 :: I, J, K, KNT, ICOUNT, IE, JE, IS, IFILP	! Modified by Rafa introducing KNT
    INTEGER(KINT)                 :: NPTS, IOS, IERR, ITR, NTCP, NIND
    INTEGER(KINT)                 :: IRANK,IRNK  !,NGCP,NGCPFR
    CHARACTER(4)                  :: IRANKD
    CHARACTER(50)                 :: DUMMY
    REAL(KREAL)                   :: X1,Y1,Z1
    ...
    allocate ( nbd(n), x(n), l(n), u(n), g(n), iwa(3*n), wa(2*m*n + 5*n + 11*m*m + 8*m), stat = ierr)
    if (ierr .ne. 0) then
        write(6,"('Error num ', i5, ' in OPTDRIVER when allocating nbd, x, l, u, g, iwa and wa in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    write(irankd,"(i4.2)") myrank
    itr=0
    open(unit=15,file=trim(filename)//".tcp_"//trim(adjustl(irankd)),access="append",status="replace", iostat=ierr)
    if (ierr .ne. 0) then
        write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") &
            ierr, trim(filename)//".tcp_"//trim(adjustl(irankd)), myrank
        abort = 1
        return
    endif

  ! All the guesscps which converge according to provided criteria are put in filename.tcp_?? file.
    ...
    do !itr= 1,5 ! Introducing iteration. All the guesscps not
        ...
        if (myrank.eq.0) then
            itr=itr+1
            open(1,file=trim(filename)//".gcp",status="old", iostat=ierr)   ! File contains already created  guess points
            if (ierr .ne. 0) then
                write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//".gcp", myrank
                abort = 1
                return
...
104			continue
        ...
        CALL MPI_BCAST(NPTS,1,MPI_INTEGER4,0,MPI_COMM_WORLD,ierr)
        CALL MPI_BCAST(itr,1,MPI_INTEGER4,0,MPI_COMM_WORLD,ierr)
        ...
        if (ierr .ne. 0) then
            write(6,"('Error num ', i5, ' in OPTDRIVER when allocating xmpi, ympi and zmpi in processor ',i3)") ierr, myrank
            abort = 1
            return
        endif
        if (myrank.eq.0) then
...
!		CALL MPI_BARRIER(MPI_COMM_WORLD,ierr)
        CALL MPI_BCAST(xmpi,NPTS,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        CALL MPI_BCAST(ympi,NPTS,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        CALL MPI_BCAST(zmpi,NPTS,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        if (.not.allocated(istav)) then
            allocate (istav(0:nprocs-1), stat = ierr)
            if (ierr .ne. 0) then
                write(iout,"('Error num ', i5, ' in OPTDRIVER when allocating istav in processor ',i3)") ierr, myrank
                abort = 1
                return
            endif
        endif
        if (.not.allocated(iendv)) then
            allocate (iendv(0:nprocs-1), stat = ierr)
            if (ierr .ne. 0) then
                write(iout,"('Error num ', i5, ' in OPTDRIVER when allocating iendv in processor ',i3)") ierr, myrank
                abort = 1
                return
            endif
        endif
! 		Breaking the total number of guess points into smaller chunks
! 		based on number of processors
        call para_range(npts)
! 		istav(myrank) and iendv(myrank) loop variables will allow each
! 		processor to run the guess points alloted to them.
!=========================================================================       
        open(unit = 16, file = trim(filename)//".gcp_"//trim(adjustl(irankd)))
        do i = istav(myrank),iendv(myrank)
...
!					and gradient g for the sample problem.
                    ...
                    if(med) then
                        CALL DAMDEN_mpi(vtot,drvxtot,drvytot,drvztot,dxxtot, &
                                &   dxytot,dxztot,dyytot,dyztot,dzztot,x1,y1,z1)
                    elseif(mesp) then
                        CALL DAMPOT_mpi(vtot,drvxtot,drvytot,drvztot,dxxtot, &
                                &   dxytot,dxztot,dyytot,dyztot,dzztot,x1,y1,z1)
                    end if
...
!       
                    ...
                    icount=icount+1
!                   Check for maximum number of steps for the convergence of a point.
...
! 	CPs which are 0.1 au away from each other are considered to be different  
    if (myrank .eq. 0) then
! ----------      change made by Rafa          -----------------------------      
        base = trim(filename)//".tcp"
        output = base
        call gather_text_files(base, output, ierr)
        if (ierr .ne. 0) then
            call error(ierr,'Error gathering files '//trim(base))
        ...
        base = trim(filename)//".gcp"
        call remove_single_file(base, ierr)
        open (17,file = trim(filename)//".tcp",status="old", iostat=ierr)
        if (ierr .ne. 0) then
            write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//".tcp", myrank
            abort = 1
            return
        endif
! ----------      End of change made by Rafa   ----------------------------  
...
106     continue
        ...
        if (ierr .ne. 0) then
            write(iout,"('Error num ', i5, ' in OPTDRIVER when allocating xmpi, ympi, zmpi and vesp in processor ',i3)") &
                    ierr, myrank
            abort = 1
            return
        endif
...
! 	Currently the array length is equal to total number of unsorted CPs 
    allocate (xscp(npts),yscp(npts),zscp(npts),vscp(npts), stat = ierr)
    if (ierr .ne. 0) then
        write(iout,"('Error num ', i5, ' in OPTDRIVER when allocating xscp, yscp, zscp and vscp in processor ',i3)") ierr, myrank
        abort = 1
        return
    endif
    if (myrank.eq.0) then
...
! 	each CPs and assinging symbols accordingly.
    ...
    CALL MPI_BCAST(NTCP,1,MPI_INTEGER4,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(xscp,NTCP,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(yscp,NTCP,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(zscp,NTCP,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(vscp,NTCP,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
    if (.not.allocated(istav)) then
        allocate (istav(0:nprocs-1), stat = ierr)
        if (ierr .ne. 0) then
            write(iout,"('Error num ', i5, ' in OPTDRIVER when allocating istav in processor ',i3)") ierr, myrank
    ...
    open(18,file=trim(filename)//".tfcp_"//trim(adjustl(irankd)), iostat=ierr)
    if (ierr .ne. 0) then
        write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") &
            ierr, trim(filename)//".tfcp_"//trim(adjustl(irankd)), myrank
        abort = 1
        return
    endif
! 	Dividing the work load among processors
    call para_range(ntcp)
    do i = istav(myrank),iendv(myrank)
        if(med) then
            CALL DAMDEN_mpi(vscp(i),drvxtot,drvytot,drvztot,dxxtot,dxytot,dxztot,dyytot,dyztot,dzztot,xscp(i),yscp(i),zscp(i))
        elseif(mesp) then
            CALL DAMPOT_mpi(vscp(i),drvxtot,drvytot,drvztot,dxxtot,dxytot,dxztot,dyytot,dyztot,dzztot,xscp(i),yscp(i),zscp(i))
        endif
...
! write(6,"('processor = ', i3, ' ntypfr = ', 4(1x,i5))") myrank, ntypfr
    CALL MPI_REDUCE (ntypfr,ntyp,4,MPI_INTEGER4,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    if (myrank.eq.0) deallocate (xmpi,ympi,zmpi,vesp)
...
! 	on top of their respective category
    if (myrank.eq.0) then
        if (med) then
            base = trim(filename)//"-cps-d.xyz"
            call remove_single_file(base, ierr)
            base = trim(filename)//"-cps-d.eigv"
            call remove_single_file(base, ierr)
        endif
        if (mesp) then
            base = trim(filename)//"-cps-v.xyz"
            call remove_single_file(base, ierr)
            base = trim(filename)//"-cps-v.eigv"
            call remove_single_file(base, ierr)
        endif
        base = trim(filename)//".tfcp"
        output = base
        call gather_text_files(base, output, ierr)
        if (ierr .ne. 0) then
            call error(ierr,'Error gathering files '//trim(base))
        endif
        allocate (symb(ntcp),xscp(ntcp),yscp(ntcp),zscp(ntcp),vscp(ntcp),eval(ntcp,3),evec(ntcp,3,3),stat=ierr)
        if (ierr .ne. 0) then
            write(6,"('Error num ', i5, ' in OPTDRIVER when allocating symb, xscp, yscp, zscp, vscp, eval and evec in processor '&
                    ,i3)") ierr, myrank
            abort = 1
            return
        endif
        ...
        if (ios .ne. 0) then
            write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ios, TRIM(filename)//".tfcp", myrank
            abort = 1
            return
        endif
...
109		continue
        ...
        sym(3)="z"
        sym(4)="m"
! 		Sorting on the basis of symbol and listing in descending order of MESP value
        nstrt(1)=1
        nstrt(2)=ntyp(1)+1
...
! ----------      change made by Rafa          -----------------------------
        if (med) then
            open(20, file=trim(filename)//"-cps-d.xyz", iostat=ierr)
            if (ierr .ne. 0) then
                write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-cps-d.xyz", myrank
                abort = 1
                return
            endif
            OPEN (55, FILE = trim(filename)//"-cps-d.eigv", iostat=ierr)
            if (ierr .ne. 0) then
                write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-cps-d.eigv", myrank
                abort = 1
                return
            endif
        else if (mesp) then
            open(20, file=trim(filename)//"-cps-v.xyz", iostat=ierr)
            if (ierr .ne. 0) then
                write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-cps-v.xyz", myrank
                abort = 1
                return
            endif
            OPEN (55, FILE = trim(filename)//"-cps-v.eigv", iostat=ierr)
            if (ierr .ne. 0) then
                write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-cps-v.eigv", myrank
                abort = 1
                return
            endif
        endif
! ----------      End of change made by Rafa   ----------------------------         
        ...
        do i = 1,4
            do j = 1, ntyp(i)
                nind = nind+1
                write(iout,"(A,1x,I4,4(3x,F15.7))") symb(nind),nind,xscp(nind),yscp(nind),zscp(nind),vscp(nind)
                write(20,"(A,4(3x,F15.7))") symb(nind),xscp(nind)*auang,yscp(nind)*auang,zscp(nind)*auang,vscp(nind)
                ifilp=55
...
!****************************************************      
    ...
    CALL MPI_BCAST(NCP,1,MPI_INTEGER4,0,MPI_COMM_WORLD,ierr)
    allocate (symcp(ncp),xcp(ncp),ycp(ncp),zcp(ncp),isym(ncp), stat = ierr)
    if (ierr .ne. 0) then
        write(iout,"('Error num ', i5, ' in GRADPATH_S when allocating symcp, xcp, ycp, zcp and isym in processor ',i3)") &
                ierr, ierr, myrank
        abort = 1
        return
    endif
...
121		rewind(21)
            close(21)
            do i= 1,ncp
                    isym(i) = ichar(symcp(i))
            end do
    end if
! 		Broadcasting the character array of CP symbols as integers.     
    if (.not.allocated(istav)) then
        allocate (istav(0:nprocs-1), stat = ierr)
        if (ierr .ne. 0) then
            write(iout,"('Error num ', i5, ' in GRADPATH_S when allocating istav in processor ',i3)") ierr, myrank
            abort = 1
            return
        endif
    endif
    if (.not.allocated(iendv)) then
        allocate (iendv(0:nprocs-1), stat = ierr)
        if (ierr .ne. 0) then
            write(iout,"('Error num ', i5, ' in GRADPATH_S when allocating iendv in processor ',i3)") ierr, myrank
            abort = 1
            return
        endif
    endif
    CALL MPI_BCAST(isym,ncp,MPI_INTEGER4,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(xcp,ncp,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(ycp,ncp,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(zcp,ncp,MPI_REAL8,0,MPI_COMM_WORLD,ierr)

    write(irankd,"(i4.2)") myrank

    open ( irmp, file=trim(filename)//".rmdat"//"_"//trim(adjustl(irankd)),status="replace")
! 	Separate temporary file for each processor to store the gradient path information.

!	Initializing Variables
    drsq=0.0; X1=0.0; Y1=0.0; Z1=0.0

!	Generate a box around the molecule wherein the gradient path will be generated---
...
! 	of electron density if less than 0.001 au
    ...
    do cpnum=istav(myrank),iendv(myrank)
        LDERIV2=.TRUE.
        if (med) then
            CALL DAMDEN_mpi(VTOT,DRVXTOT,DRVYTOT,DRVZTOT,DXXTOT,DXYTOT,DXZTOT,DYYTOT,DYZTOT,DZZTOT,   &
                                    & XCP(cpnum),YCP(cpnum),ZCP(cpnum))
        elseif (mesp) then
            CALL DAMPOT_mpi(VTOT,DRVXTOT,DRVYTOT,DRVZTOT,DXXTOT,DXYTOT,DXZTOT,DYYTOT,DYZTOT,DZZTOT,   &
                                    & XCP(cpnum),YCP(cpnum),ZCP(cpnum))
        end if
        CALL HESSIAN(IRANK,IRNK,D,V,drvxtot,drvytot,drvztot,dxxtot,dxytot,dxztot,dyytot,dyztot,dzztot)
        ...
        do j= 1,2  ! when j=1, the gradient path is followed in the direction of the gradient
            do i= 1,6  ! i=1,3 Moves in the direction of eigenvector, while i=4,6 moves in the opposite direction of the eigenvector
               ...
               write(IRMP,8)x1,y1,z1
                do while (flag==0)
                    if (med) then
                            call damden_mpi(vtot,drvxtot,drvytot,drvztot,dxxtot,dxytot,dxztot,dyytot,dyztot,dzztot,x1,y1,z1)
                    elseif (mesp) then
                            call dampot_mpi(vtot,drvxtot,drvytot,drvztot,dxxtot,dxytot,dxztot,dyytot,dyztot,dzztot,x1,y1,z1)
                    end if
!					rationalise
...
! 					xpp,ypp,zpp,xp,yp and zp are also transferred to terminate subroutine.
                    call terminate(vtot, drvxtot, drvytot, drvztot, x1, y1, z1, IRMP)
                end do
            end do
        end do
    end do ! End of loop pver number of CPs, from where the gradient path originate.
8	format(3(f6.2,1x))                  
...
! 		directions of all the CPs.
        ...
        allocate (symcp1(NCP*12),ncp1(NCP*12),symcp2(NCP*12),ncp2(NCP*12),ngp(NCP*12),NRMA(0:NCP*12),IND(NCP*12), stat = ierr)
        if (ierr .ne. 0) then
            write(iout,"('Error num ', i5, ' in GRADPATH_S when allocating symcp1, ncp1, symcp2, ncp2, ngp, nrma and ind&
                    &in processor ',i3)") ierr, myrank
            abort = 1
            return
        endif
...
! ----------      change made by Rafa          -----------------------------        
        if (med)  then
            OPEN(IFILA,FILE=trim(filename)//"-d.gpdat",access="append",status="replace", iostat=ierr)
            if (ierr .ne. 0) then
                write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-d.gpdat", myrank
                abort = 1
                return
            endif
            if (basin) then
                OPEN(12,FILE=TRIM(filename)//"-d.cnct",status="replace", iostat=ierr)
                if (ierr .ne. 0) then
                    write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-d.cnct", myrank
                    abort = 1
                    return
                endif
            endif
        elseif (mesp) then
            OPEN(IFILA,FILE=trim(filename)//"-v.gpdat",access="append",status="replace", iostat=ierr)
            if (ierr .ne. 0) then
                write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-v.gpdat", myrank
                abort = 1
                return
            endif
            if (basin) then
                OPEN(12,FILE=TRIM(filename)//"-v.cnct",status="replace", iostat=ierr)
                if (ierr .ne. 0) then
                    write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-d.cnct", myrank
                    abort = 1
                    return
...
119         close(102)  ! Closing rmdat file
        ...
        base = trim(projectname)//".rmdat"
        call remove_single_file(base, ierr)
        deallocate (symcp1,ncp1,symcp2,ncp2,ngp,nrma,ind)
    end if  ! Ending if statement for proc id 0.
    deallocate(isym,xcp, ycp,zcp,symcp)
    return
    end subroutine
!---------------------------------------------------------------------------------
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    if (med) then
        if (vtot.lt.0.0005) then  ! Takes the density value of 0.001 au to be the cutoff for the gradient path
            write(IRMP,*) char(isym(cpnum)),cpnum,"ob",cpnum,cnt
            FLAG=1
            return
        end if
...
!				write(iout,"(2(a,1x,a,1x,I3,1x),I7)") "Gradient path connects",symcp(cpnum),cpnum,"to",symcp(l),l, cnt
                    write(IRMP,*) char(isym(cpnum)),cpnum,char(isym(l)),l,cnt
                    FLAG=1
                    return
                end if
            elseif (l.eq.cpnum) then
                if ((abs(X1-XCP(l))<DISTcp).AND.(abs(Y1-YCP(l))<DISTcp).AND.(abs(Z1-ZCP(l))<DISTcp)) then
                    write(IRMP,*) char(isym(cpnum)),cpnum,"ob",l,cnt
                    FLAG=1
                    return
                end if
            end if
    ...
    if ((abs(drvxtot)>drcutnuc).OR.(abs(drvytot)>drcutnuc).OR.(abs(drvztot)>drcutnuc)) then
        do l=1, NCEN
            if ((abs(X1-RCEN(1,l))<DISTnuc).AND.(abs(Y1-RCEN(2,l))<DISTnuc).AND.(abs(Z1-RCEN(3,l))<DISTnuc)) then
                write(irmp,*) char(isym(cpnum)),cpnum,atmnam(l),l,cnt
                FLAG=1
                return
            end if
        end do
...
!	If the path finds a loop
    ...
    if((ABS(xpp-x1).lt.1.0E-07).AND.(ABS(ypp-y1)<1.0E-07) .AND. (ABS(zpp-z1)<1.0E-07)) then
        write(IRMP,*) char(isym(cpnum)),cpnum,"ob",cpnum,cnt
        FLAG=1
        return
    end if
...
!****************************************************      
    ...
    INTEGER(KINT)               :: iars, npar, noen, noenmax, ndots1, ndots1s, kl, ins, jns, kntvert, kntverttot, kntind ! modified by Rafa 2019
    INTEGER(KINT), DIMENSION(200):: ndotss
    INTEGER(KINT), DIMENSION(200):: ndots
    REAL(KREAL), PARAMETER     :: pival = 3.14159265359
...
! ----------      change made by Rafa          -----------------------------
        if(med) then
            open(25,file=trim(filename)//"-cps-d.xyz",status="old", iostat=ierr)
            if (ierr .ne. 0) then
                write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-cps-d.xyz", myrank
                abort = 1
                return
        ...
        elseif(mesp) then
            open(25,file=trim(filename)//"-cps-v.xyz",status="old", iostat=ierr)
            if (ierr .ne. 0) then
                write(6,"('Error num ', i4, ' opening file ', a, ' in proc',i5)") ierr, trim(filename)//"-cps-v.xyz", myrank
                abort = 1
                return
...
! ----------      End of change made by Rafa   ----------------------------  
    ...
    allocate(symcp(ncp),xcp(ncp),ycp(ncp),zcp(ncp),isym(ncp), stat = ierr)
    if (ierr .ne. 0) then
        write(iout,"('Error num ', i5, ' in BASIN_S when allocating symcp, xcp, ycp, zcp and isym in processor ',i3)") &
                ierr, myrank
        abort = 1
        return
    endif
...
185         rewind(25)
    ...
    if (.not.allocated(istav)) then
        allocate (istav(0:nprocs-1), stat = ierr)
        if (ierr .ne. 0) then
            write(iout,"('Error num ', i5, ' in BASIN_S when allocating istav in processor ',i3)") ierr, myrank
            abort = 1
            return
        endif
    endif
    if (.not.allocated(iendv)) then
        allocate (iendv(0:nprocs-1), stat = ierr)
        if (ierr .ne. 0) then
            write(iout,"('Error num ', i5, ' in BASIN_S when allocating iendv in processor ',i3)") ierr, myrank
            abort = 1
            return
        endif
    endif
...
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
      ...
      open (unit=IBILQT+1, file=trim(filename)//".vert"//"_"//trim(adjustl(irankd)),status="replace", iostat=ierrqt)
      if (ierrqt .ne. 0) then
          write(6,*) 'Error when opening file ', trim(filename)//".vert"//"_"//trim(adjustl(irankd))
          abort = 1
          return
      endif
!        End of modification by Rafa 2019  ----------------------------------------------------------------------------------

    IF (SOLIDSURF) OPEN(IBILS,file=trim(filename)//".ssurf"//"_"//trim(adjustl(irankd)),status="replace")
    IF (WIREFRAME) OPEN(IBILW,file=trim(filename)//".wsurf"//"_"//trim(adjustl(irankd)),status="replace")
...
! 	Basin will be created usind the eigenvectors of (3,-1) CP ==> z CP only.
    DO I = 1, NCP
        if (char(isym(i)).eq."z") NZCP = NZCP + 1
!                Bug fixed by Rafa (September 2017)
!		if (char(isym(i)).ne."z") NXYCP = NXYCP + 1  Wrong test (m CP's come after z CP's and must not be accounted)
        if (char(isym(i)).ne."z" .and. char(isym(i)).ne."m") NXYCP = NXYCP + 1
    END DO
!write(iout, "('ncp = ', i3, ' nzcp = ', i3, ' NXYCP = ', i3)") ncp, nzcp, NXYCP

    CALL PARA_RANGE(NZCP)

    kntvert = 0 ! modified by Rafa 2019
    do cpnum = istav(myrank)+nxycp,iendv(myrank)+nxycp
        IF (WIREFRAME) write(ibilw,*)char(isym(cpnum)),cpnum
        IF (SOLIDSURF) write(ibils,*)char(isym(cpnum)),cpnum
        lderiv2=.true.
        if (med) then
                call damden_mpi(vtot,drvxtot,drvytot,drvztot,dxxtot,dxytot,dxztot,dyytot,dyztot,dzztot,   &
                                        & xcp(cpnum),ycp(cpnum),zcp(cpnum))
        ...
        call hessian(irank,irnk,d,v,drvxtot,drvytot,drvztot,dxxtot,dxytot,dxztot,dyytot,dyztot,dzztot)
        ! hessian diagonalizes the hessian matrix formed by dxx,dxy... dzz
        ! and find eigenvalues as well as eigenvectors
        do i = 1, 3
                val(i)= d(i)
        end do
!        write(iout,"(/'cp n0 = ', i3, ' coords = ', 3(1x,e22.15))") cpnum, xcp(cpnum),ycp(cpnum),zcp(cpnum)
!        write(iout,"('eigenvalues = ', 3(1x,e22.15))") val(1:3)
!        write(iout,"('eigenvectors of cp num', i3)") cpnum

        do i = 1, 3
            do j = 1, 3
                vec(i,j) = v(i,j)
            end do
!            write(iout,"(3(1x,e22.15))") vec(i,1:3)
        end do
...
! 		the lying on the eigenvector, making a distance of fdisp away from CP.
            ...
            do while (flag==0 .and. noen .lt. noenmax)   ! modified by Rafa 2019
                ...
                if ((cnt1*step)>=0.2) then ! Print points only when you have moved 0.2 au from previous point
                    ...
                    if (i.eq.1) then
                        ...
                        yline1(noen)=y1
                        zline1(noen)=z1
                        if (noen.eq.3) then  ! Noen equals 3 is chosen to
! 							ensure that the path has acquired a considerable bend 
                            Vecx = x1-xcp(cpnum)
                            Vecy = y1-ycp(cpnum)
                            Vecz = z1-zcp(cpnum)
...
!        Writes vertices and indices to basisfile for DAMQT
                    if (ierrqt .eq. 0) then
                        write(ibilqt+1,"(18(e12.5,1x))") xcp(cpnum),ycp(cpnum),zcp(cpnum), vnx(1), vny(1), vnz(1), &
...
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
                        if (lenths.le.exln) then
                            IF (SOLIDSURF) write(ibils,89) xlines(1,ins),ylines(1,ins),zlines(1,ins),&   ! modified by Rafa 2019
                                  & xlines(2,ins),ylines(2,ins),zlines(2,ins),&
                                  & xlines(1,ins+1),ylines(1,ins+1),zlines(1,ins+1),&
                                  & vnx(1),vny(1),vnz(1),vnx(2),vny(2),vnz(2),&
                                  & vnx(3),vny(3),vnz(3)
!        Writes vertices and indices to basisfile for DAMQT
                            if (ierrqt .eq. 0) then
                                write(ibilqt+1,"(18(e12.5,1x))") xlines(1,ins),ylines(1,ins),zlines(1,ins), &
                                    vnx(1), vny(1), vnz(1), &
...
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
                        if (lenths.le.exln) then
                            IF (SOLIDSURF) write(ibils,89) xlines(2,ins),ylines(2,ins),zlines(2,ins),&   ! modified by Rafa 2019
                                  & xlines(2,ins+1),ylines(2,ins+1),zlines(2,ins+1),&
                                  & xlines(1,ins+1),ylines(1,ins+1),zlines(1,ins+1),&
                                  & vnx(1),vny(1),vnz(1),vnx(2),vny(2),vnz(2),&
                                  & vnx(3),vny(3),vnz(3)
!        Writes vertices and indices to basisfile for DAMQT
                            if (ierrqt .eq. 0) then
                                write(ibilqt+1,"(18(e12.5,1x))") xlines(2,ins),ylines(2,ins),zlines(2,ins), &
                                    vnx(1), vny(1), vnz(1), &
...
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
                            if (lenths.le.exln) then
                                IF (SOLIDSURF) write(ibils,89) xlines(1,ndotss(1)),ylines(1,ndotss(1)),&   ! modified by Rafa 2019
                                      & zlines(1,ndotss(1)),&
                                      & xlines(2,ins),ylines(2,ins),zlines(2,ins),&
                                      & xlines(2,ins+1),ylines(2,ins+1),zlines(2,ins+1),&
                                      & vnx(1),vny(1),vnz(1),vnx(2),vny(2),vnz(2),&
...
!        Writes vertices and indices to basisfile for DAMQT
                                  if (ierrqt .eq. 0) then
                                      write(ibilqt+1,"(18(e12.5,1x))") xlines(1,ndotss(1)), &
                                          ylines(1,ndotss(1)), zlines(1,ndotss(1)), &
                                          vnx(1), vny(1), vnz(1), &
                                          xlines(2,ins),ylines(2,ins),zlines(2,ins), vnx(2), vny(2), vnz(2), &
...
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
                            if (lenths.le.exln) then
                                IF (SOLIDSURF) write(ibils,89) xlines(2,ndotss(2)),ylines(2,ndotss(2)),&   ! modified by Rafa 2019
                                      & zlines(2,ndotss(2)),&
                                      & xlines(1,ins+1),ylines(1,ins+1),zlines(1,ins+1),&
                                      & xlines(1,ins),ylines(1,ins),zlines(1,ins),&
                                      & vnx(1),vny(1),vnz(1),vnx(2),vny(2),vnz(2),&
...
!        Writes vertices and indices to basisfile for DAMQT
                                  if (ierrqt .eq. 0) then
                                      write(ibilqt+1,"(18(e12.5,1x))") xlines(2,ndotss(2)), &
                                          ylines(2,ndotss(2)),zlines(2,ndotss(2)), &
                                          vnx(1), vny(1), vnz(1), &
...
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
                        if (lenths.le.exln) then
                            IF (SOLIDSURF) write(ibils,89) xline1s(ins),yline1s(ins),zline1s(ins),&   ! modified by Rafa 2019
                                  & xline1s(ins+1),yline1s(ins+1),zline1s(ins+1),&
                                  & xlines(1,ins+1),ylines(1,ins+1),zlines(1,ins+1),&
                                  & vnx(1),vny(1),vnz(1),vnx(2),vny(2),vnz(2),&
                                  & vnx(3),vny(3),vnz(3)
!        Writes vertices and indices to basisfile for DAMQT
                              if (ierrqt .eq. 0) then
                                  write(ibilqt+1,"(18(e12.5,1x))") xline1s(ins),yline1s(ins),zline1s(ins), vnx(1), vny(1), vnz(1), &
...
!        Writes vertices and indices to basisfile for DAMQT
                                if (ierrqt .eq. 0) then
                                    write(ibilqt+1,"(18(e12.5,1x))") xlines(1,ndotss(1)),ylines(1,ndotss(1)), zlines(1,ndotss(1)), &
                                        vnx(1), vny(1), vnz(1), &
                                        xline1s(ins), yline1s(ins), zline1s(ins), vnx(2), vny(2), vnz(2), &
...
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
                            if (lenths.le.exln) then
                                IF (SOLIDSURF) write(ibils,89) xline1s(ndots1s),yline1s(ndots1s),&   ! modified by Rafa 2019
                                      & zline1s(ndots1s),&
                                      & xlines(1,ins+1),ylines(1,ins+1),zlines(1,ins+1),&
                                      & xlines(1,ins),ylines(1,ins),zlines(1,ins),&
                                      & vnx(1),vny(1),vnz(1),vnx(2),vny(2),vnz(2),&
...
!        Writes vertices and indices to basisfile for DAMQT
                              if (ierrqt .eq. 0) then
                                  write(ibilqt+1,"(18(e12.5,1x))") xline1s(ndots1s),yline1s(ndots1s), zline1s(ndots1s), &
                                      vnx(1), vny(1), vnz(1), &
                                      xlines(1,ins+1),ylines(1,ins+1),zlines(1,ins+1), vnx(2), vny(2), vnz(2), &
                                      xlines(1,ins),ylines(1,ins),zlines(1,ins), vnx(3), vny(3), vnz(3)
                                  kntvert = kntvert + 3
...
987				continue   
    ...
    CALL MPI_REDUCE(kntvert,kntverttot,1,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    close (IBILQT+1)
!        Modified by Rafa 2019  -------------------------------------------------------------------------------------------
    if (myrank .eq. 0) then
        base = trim(filename)//".vert"
        output = base
        call gather_text_files(base, output, ierr)
        if (ierr .ne. 0) then
            call error(ierr,'Error gathering files '//trim(base))
        ...
        open (unit=IBILQT+1, file=trim(filename)//".vert",iostat=ierrqt)
        if (ierrqt .ne. 0) then
            ...
            rewind (IBILQT+1)
            write(IBILQT) kntverttot
            do kntind = 1, kntverttot / 3
...
CALL MPI_BARRIER(MPI_COMM_WORLD,ierr)
    ...
    if (myrank.eq.0) then
        ierr = 0
        if (solidsurf) then     ! solid surface
            base = trim(filename)//".ssurf"
            if (med) then
                output = trim(filename)//"-d.bssol"
            else
                output = trim(filename)//"-v.bssol"
            endif
            call gather_text_files(base, output, ierr)
        else if (wireframe) then   ! wireframe surface
            base = trim(filename)//".wsurf"
            if (med) then
                output = trim(filename)//"-d.bswir"
            else
                output = trim(filename)//"-v.bswir"
            endif
            call gather_text_files(base, output, ierr)
        endif
        if (ierr .ne. 0) then
...
! 					And the gradient path has reached within 0.05 au or less to another CP then  
	...
	if ((abs(drvxtot)>drcutnuc).OR.(abs(drvytot)>drcutnuc).OR.(abs(drvztot)>drcutnuc)) then
            do l=1, NCEN
                if ((abs(X1-RCEN(1,l))<DISTnuc).AND.(abs(Y1-RCEN(2,l))<DISTnuc).AND.(abs(Z1-RCEN(3,l))<DISTnuc)) then
!				write(irmp,*) char(isym(cpnum)),cpnum,atmnms(int(NCN(l))),l,cnt 
                    FLAG=1
                    return
                end if
            end do
...
!	If the path finds a loop
	...
	if((ABS(xpp-x1).lt.1.0E-07).AND.(ABS(ypp-y1)<1.0E-07) .AND. (ABS(zpp-z1)<1.0E-07)) then
!		write(iout,*) "The path stuck in a loop at", xpp, ypp, zpp, "and", x1, y1, z1
            FLAG=1
            return
	end if
	FLAG=0
...
! ---------------     End of change made by Rafa   -------------- 
	...
	write(iout,"(a)")"The code invokes OPEN-MPI libraries for parallel computation"    
	write(iout,"(a)")""    
	write(iout,"(a)")""    
...
! ---------------     End of change made by Rafa   --------------       
	...
	write(iout,"(a)")"Symbols viz. x,y,z and m (small letters) are &
				& used to denote the"    
