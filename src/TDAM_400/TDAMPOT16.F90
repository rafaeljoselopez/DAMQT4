...
!
  subroutine leedamqtpot
    ...
    if (ierr .ne. 0) then
        ...
        call error(ierr,"file inquiry error")
        return
    endif
    if (nsize .eq. -1) then
    ...
    if (ierr .ne. 0) then
        ...
        call error(ierr,"Error opening damqt file")
        return
    endif
#if _WIN32
...
!        Allocates memory for the basis set
        ...
        if (ierr .ne. 0) then
            call error(ierr,"Memory error when allocating ll, lmaxc, nf, ngini, ngfin, nn, rnor or xx")
            return
        endif
...
!                computes and stores the radial normalization factor
                ...
                icarga = icarga+nprimit(knt)    ! actualizes the index for loading primitives exponents and contraction 
                                                ! coefficients
            enddo
        enddo
...
!    Data of density representation
    ...
    if (ierr .ne. 0) then
        call error(ierr,"Memory error when allocating icfposd")
        return
    endif
    ...
    if (ierr .ne. 0) then
        call error(ierr,"Memory error when allocating xajustd")
        return
    endif
...
!    Reads auxiliary integrals from file .dmqtv
    ...
    if (ierr .ne. 0) then
        call error(ierr,"Memory error when allocating cfrint1, cfrint2l2, QGacum, Qgpart, qpacum, qppart, rlargo and rmultip")
        return
    endif
    cfrint1 = cero
    cfrint2l2 = cero
...
!    Reads the fitting coeficients of auxiliary integrals for electrostatic potential and field
        ...
        read(11) cfrint2l2(icfposd(1,ia):icfposd(lmtop*nintervaj+1,ia)-1)   ! Expansion coefficients of auxiliary integrals 
                                                                            ! rint2l2
    enddo
    close(10)
    close(11)
...
!    Determines the long-range radii and the highest l in the expansion for each interval
    ...
    if (ierr .ne. 0) then
        call error (ierr,"Memory error when allocating lcorto, llargo, auxll,  and Qllargo")
        return
    endif
!    long-range radii
