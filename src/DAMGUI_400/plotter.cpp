...
void Plotter::updatecontourlabels()
{
    if (contourplot){
        ...
        int lm = gridLeftMargin;
        int tm = gridTopMargin;
        double dltx = canvassize.width() - lm - gridRightMargin;
        double dlty = canvassize.height() - tm - gridBottomMargin;
...
void Plotter::paintEvent(QPaintEvent * /* event */)
{
    ...
    QRect rect(gridLeftMargin+1, gridTopMargin,
        canvassize.width() - gridLeftMargin - gridRightMargin, canvassize.height() - gridTopMargin - gridBottomMargin);
...
void Plotter::capture_image()
{
    ...
    if (fradplot && showcurvelabels){
        ...
        if (plotbonds){
            ...
            painter.setPen(pen);
            PlotSettings settings = zoomStack[curZoom];
            QRect rect(gridLeftMargin, gridTopMargin,
                       canvassize.width() - gridLeftMargin - gridRightMargin,
                       canvassize.height() - gridTopMargin - gridBottomMargin);
            QPoint r1;
            QPoint r2;
            for (int i = 0 ; i < bonds->count() ; i +=2){
...
void Plotter::updateplot()
{
    ...
    gridTopMargin = zoomStack[curZoom].TopMargin;
    gridBottomMargin = zoomStack[curZoom].BottomMargin;
    gridLeftMargin = zoomStack[curZoom].LeftMargin;
    gridRightMargin = zoomStack[curZoom].RightMargin;
    int Ylabelshift;
    Ylabelshift = int(5 + 2*QFontMetrics(fontYlabel).xHeight());
    gridLeftMargin += Ylabelshift;
#if QT_VERSION < 0x050B00
    if (getshowYscaleleft()){
        gridLeftMargin += int(QFontMetrics(fontYlabel).width(QString("0"))
            *(QString::number(settings.minY+settings.spanY(),fy,zoomStack[curZoom].ncifrasy).length()+1));
        Ylabelshift += int(QFontMetrics(fontYlabel).width(QString("0"))
            *(QString::number(settings.minY+settings.spanY(),fy,zoomStack[curZoom].ncifrasy).length()+1));
    }
    if (getshowYscaleright()){
        gridRightMargin += int(QFontMetrics(fontYlabel).width(QString("0"))
            *(QString::number(settings.minY+settings.spanY(),fy,zoomStack[curZoom].ncifrasy).length()+1));
    }
#else
    int yscalewidth = int(QFontMetrics(fontYlabel).horizontalAdvance(QString("0"))
                *(QString::number(settings.minY+settings.spanY(),fy,zoomStack[curZoom].ncifrasy).length()+1));
    int upperxscaleheight;
    if (showXscaletop){
        upperxscaleheight = int(QFontMetrics(fontXlabel).boundingRect('0').height());
    }
    else{
        upperxscaleheight = 0;
    }
    int lowerxscaleheight;
    if (showXscalebottom){
        lowerxscaleheight = int(QFontMetrics(fontXlabel).boundingRect('0').height());
    }
    else{
        lowerxscaleheight = 0;
    }
    if (getshowYscaleleft()){
        gridLeftMargin += yscalewidth;
    }
    if (getshowYscaleright()){
        gridRightMargin += int(QFontMetrics(fontYlabel).horizontalAdvance(QString("0"))
            *(QString::number(settings.minY+settings.spanY(),fy,zoomStack[curZoom].ncifrasy).length()+1));
    }
#endif

    gridBottomMargin += int(10 + 2*QFontMetrics(fontXlabel).xHeight());
    if (getshowXscalebottom()){
        gridBottomMargin += int(10 + 2*lowerxscaleheight);
    }
    gridTopMargin += int(10 + 2*QFontMetrics(fontXlabel).xHeight());
    if (getshowXscaletop()){
        gridTopMargin += int(10 + 2*upperxscaleheight);
    }

    if (aspectratio){
        double w = canvassize.rwidth();
        double h = canvassize.rheight();
        double deltax = (w - gridLeftMargin - gridRightMargin) / (zoomStack[curZoom].maxX - zoomStack[curZoom].minX);
        double deltay = (h - gridTopMargin - gridBottomMargin) / (zoomStack[curZoom].maxY - zoomStack[curZoom].minY);
        if (deltax > deltay){
            gridLeftMargin = (int) (0.5*(w - deltay * (zoomStack[curZoom].maxX - zoomStack[curZoom].minX)));
            gridRightMargin = gridLeftMargin;
        }
        else if (deltax < deltay){
            gridTopMargin = (int) (0.5*(h - deltax * (zoomStack[curZoom].maxY - zoomStack[curZoom].minY)));
            gridBottomMargin = gridTopMargin;
        }
    }

    titlerect.moveCenter(QPoint((canvassize.width()+gridLeftMargin-gridRightMargin)/2,
            std::max<int>(titlerect.height(),gridTopMargin-titlerect.height()-2*upperxscaleheight)));
    emit titleposChanged();
    Xlabelrect.moveCenter(QPoint((canvassize.width()+gridLeftMargin-gridRightMargin)/2,
            std::min<int>(canvassize.height()-Xlabelrect.height(),canvassize.height()-gridBottomMargin+Xlabelrect.height()+2*lowerxscaleheight)));
    emit XlabelposChanged();
    Ylabelrect.moveCenter(QPoint(std::max<int>(gridLeftMargin - Ylabelrect.width()/2 - yscalewidth,
            Ylabelshift), (canvassize.height()+Ylabelrect.height())/2));
    ...
    if (fradplot && showcurvelabels && curvelabelrect != nullpointer){
        for (int i = 0 ; i < curvelabelrect->count() ; ++i){
            (curvelabelrect->operator [](i)).moveCenter(QPoint(4*(canvassize.width())/5,TopMargin+(i+1)*Ylabelrect.height()));
        }
    }
    if (sghistplot && showsghistlabels && sghistlabelrect != nullpointer){
        ...
        for (int i = 0 ; i < sghistlabelrect->count() ; ++i){
            if (!sghisthaslabel->at(i))
                continue;
            (sghistlabelrect->operator [](i)).moveCenter(QPoint(4*(canvassize.width())/5,TopMargin+(id++)*Ylabelrect.height()));
        }
    }
    if(fieldplot){
        updateallcenterslabelsrect();
...
void Plotter::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->button() == Qt::RightButton && (contourplot || fieldplot)){
        ...
        doty = event->y();
        double u, v;
        u = zoomStack[curZoom].minX + (dotx - gridLeftMargin) * (zoomStack[curZoom].maxX - zoomStack[curZoom].minX)
                / (canvassize.width() - gridLeftMargin - gridRightMargin );
        v = zoomStack[curZoom].maxY + (doty - gridTopMargin) * (zoomStack[curZoom].minY - zoomStack[curZoom].maxY)
                / (canvassize.height() - gridTopMargin - gridBottomMargin );
...
void Plotter::mouseReleaseEvent(QMouseEvent *event)
{
    if ((event->button() == Qt::LeftButton) && rubberBandIsShown) {
        ...
        if (rect.width() < 4 || rect.height() < 4)
            return;
        rect.translate(-gridLeftMargin, -gridTopMargin);

        PlotSettings prevSettings = zoomStack[curZoom];
        PlotSettings settings;
        double dx = prevSettings.spanX() / (double)(canvassize.width() - gridLeftMargin - gridRightMargin);
        double dy = prevSettings.spanY() / (double)(canvassize.height() - gridTopMargin - gridBottomMargin);
...
void Plotter::mouseDoubleClickEvent(QMouseEvent *event)
{
    ...
    if (contourplot && showcontourlabels){ 
        if(x > LeftMargin && x < canvassize.width()-RightMargin && y > TopMargin && y < canvassize.height()-BottomMargin){
            double u, v;
            u = zoomStack[curZoom].minX + (x - gridLeftMargin) * (zoomStack[curZoom].maxX - zoomStack[curZoom].minX)
                    / (canvassize.width() - gridLeftMargin - gridRightMargin );
            v = zoomStack[curZoom].maxY + (y - gridTopMargin) * (zoomStack[curZoom].minY - zoomStack[curZoom].maxY)
                    / (canvassize.height() - gridTopMargin - gridBottomMargin );
            double iu = (u - umincnt) * (nu-1) / (umaxcnt - umincnt);
            double iv = (v - vmincnt) * (nv-1) / (vmaxcnt - vmincnt);
if (iu < 0 || iv < 0 || iu > canvassize.width() || iv > canvassize.height()){
qDebug() << "WARNING!!!: iu = " << iu << " iv = " << iv << " umincnt = " << umincnt
    << " umaxcnt = " << umaxcnt << " vmincnt = " << vmincnt << " vmaxcnt = " << vmaxcnt;
}
    ...
    if (fradplot && showcurvelabels && curvelabelrect != nullpointer){
        ...
        if(x > gridLeftMargin && x < canvassize.width() - gridRightMargin
                && y > gridTopMargin && y < canvassize.height() - gridBottomMargin){
...
//    caseplot == 4: 1D MESP sigma hole histogram
    PlotSettings settings = zoomStack[curZoom];
    QRect rect(gridLeftMargin+1, gridTopMargin,
               canvassize.width() - gridLeftMargin - gridRightMargin, canvassize.height()
               - gridTopMargin - gridBottomMargin);
    if (!rect.isValid())
        return;
...
void Plotter::setcenterlabelrect(){
    ...
    QRect rect(settings.LeftMargin, settings.TopMargin,
               canvassize.width() - settings.LeftMargin - settings.RightMargin,
               canvassize.height() - settings.TopMargin - settings.BottomMargin);
    if (!rect.isValid())
        return;
