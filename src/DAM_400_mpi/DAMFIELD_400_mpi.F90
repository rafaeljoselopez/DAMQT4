...
! DAM_400. Version parallelized with MPI only 3D lines.
!
! Version of April 2019
!
  program DAMFIELD_400_mpi
    USE MPI
    USE DAM_400_D
    USE DAM_400_CONST_D
    USE DAM_400_DATA_D
...
!	Namelist default values
    ...
    filename = ""       ! root file name for .field3D files
...
!	Opens file for field lines
    ...
    open (unit=12, file=trim(filename)//".field3D_"//trim(adjustl(strbux)),status='unknown',form='formatted',iostat=ierr)
    if (ierr .ne. 0) then
        write(6,"('Error in processor ',i3, ' when opening file ', a, '.Stop')") myrank, &
            trim(filename)//".field3D_"//trim(adjustl(strbux))
        abort = 1
    endif
    CALL MPI_REDUCE(abort,abortroot,1,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(abortroot,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
