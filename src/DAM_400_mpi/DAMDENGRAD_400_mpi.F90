...
!
  program DAMDENGRAD_400_mpi
    USE MPI
    USE DAM_400_D
    USE DAM_400_CONST_D
    USE DAM_400_DATA_D
    USE DAMQT_UTILS
    USE DAMDENGRAD_400_D
    USE icosahedron
    USE PARALELO
    implicit none
...
!     Namelist default values
    ...
    filename = ""       ! root file name for .dengr3D files
...
!     Opens file for density gradient lines    
    ...
    open (unit=12, file=trim(filename)//".dengr3D_"//trim(adjustl(strbux)),status='unknown',form='formatted',iostat=ierr)
    if (ierr .ne. 0) then
        write(6,"('Error in processor ',i3, ' when opening file ', a, '.Stop')") myrank, &
            trim(filename)//".dengr3D_"//trim(adjustl(strbux))
        abort = 1
    endif
    CALL MPI_REDUCE(abort,abortroot,1,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierr)
    CALL MPI_BCAST(abortroot,1,MPI_LOGICAL,0,MPI_COMM_WORLD,ierr)
