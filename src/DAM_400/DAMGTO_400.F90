...
!	Reads geometry, basis set and density matrix generated by an external program 
    lgbsgz = .false.	! Checks whether the .sgbs file is gzipped or not
...
!	writes geometry in .xyz file for gOpenMol compatibility: coordinates are in Angstrom
!
...
!   ***************************************************************
  ...
  subroutine frgsiggauss(lf0, i1, i2, rab)
    ...
    USE GAUSS
    implicit none
    integer(KINT) :: i, ii, i1, i1ini, i1fin, i2, i2ini, i2fin, icarga, ipunt, j, jfinal, jj, k, kmax
...
!	regular spherical harmonics
    ...
    allocate(app(0:2*mxltot+1,0:mxkcof))
    if (.not. allocated(app)) call error(1,'Memory error when allocating app. Stop')
    if (longoutput) write(6,"('Size of app   = ', i15, ' bytes')") size(app)
    allocate(bpp(0:2*mxltot+1,0:mxkcof))
    if (.not. allocated(bpp)) call error(1,'Memory error when allocating bpp. Stop')
...
!	Polynomials P_k^(L,M;L',M')(0,0,1) of the shift-operators technique in the alligned-axes system
    ...
    allocate(ipntpolP(0:mxl+lmaxexp,0:mxl), stat = ierr)	! Pointers to the elements P_0^(L,0;L',0)
    if (ierr .ne. 0) call error(1,'Error allocating ipntpolP. Stop')

    n = (mxl+1) * (mxl+2) * (mxl+3) * (mxl+4) / 24 + lmaxexp * (mxl+1) * (mxl+2) * (mxl+3) / 6

    allocate(polP(n), stat = ierr)
    if (ierr .ne. 0) call error(1,'Error allocating polP. Stop')
