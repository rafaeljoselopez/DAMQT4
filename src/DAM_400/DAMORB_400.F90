...
!	generated by SMILES. These "extra" data are just skipped)
    ...
    open(15,file=trim(projectname)//".sgbs",form='unformatted',status='old',iostat=ierr)
    if (ierr .ne. 0) then
        call error(1,'Cannot open file'//trim(projectname)//".sgbs"//'. Stop')
        return
    endif
    rewind(15)
    read(15) umbrznm2c
...
!	Reads the number of centers
    ...
    if (ierr .ne. 0) then
        call error(1,'Cannot open file'//trim(projectname)//".ggbs"//'. Stop')
        return
    endif
    read(15,*) ncen
...
!			actualizes the index for loading
    ...
    if (ncaps .gt. mxcap) then
        ...
        write(6,"('Modify parameter  mxcap  in module DAM_400_D of file DAM_400_GLOBAL.F90 and recompile.')")
        call error(1,' Stop')
    endif
    if (lmaxbase .gt. mxl) then
        write(6,"('Basis functions with not allowed values of  l. ')")
