...
!	Allocates arrays
    ...
    if (myrank .eq. 0) then
        ...
        if (ierr .ne. 0) then
                write(6,"('Memory error when allocating array in gridexact in processor ',i3)") myrank
                abort = 1
                return
        endif
        iuni = 1
...
! 	arrayranksp = cero
    ...
    if (ierr .ne. 0 .and. myrank .eq. 0) then
        write(6,"('Error in MPI_GATHERV for arrayrank,   ierr = ',i7)") ierr
        abort = 1
        return
    endif
    if (myrank .eq. 0) then
...
!	Allocates arrays and opens files for molecular density tabulation
    ...
    if (lminrep .eq. 1) then
        allocate(qdefpos(0:nsel), qdefneg(0:nsel), qdefposrank(0:nsel), qdefnegrank(0:nsel), stat = ierr)
        if (ierr .ne. 0) then
            write(6,"('Memory error when allocating qdefpos(0:nsel), qdefneg(0:nsel), qdefposrank&
                & and qdefnegrank in gridrep in processor ',i3)") myrank
            abort = 1
            return
        endif
    endif
    ...
    if (latomics) then
        allocate(arrayat(nxyz,nsel), arrayatrank(nxyzrank,nsel), stat = ierr)
        if (ierr .ne. 0) then
            write(6,"('Memory error when allocating arrayat and arrayatrank in gridrep in processor ',i3)") myrank
            abort = 1
            return
        endif
        arrayatrank = cero	! Array initialization
        if (lgradient) then
            allocate(arrayatdx(nxyz,nsel), arrayatdy(nxyz,nsel), arrayatdz(nxyz,nsel), &
                arrayatdxrank(nxyzrank,nsel), arrayatdyrank(nxyzrank,nsel), arrayatdzrank(nxyzrank,nsel), stat = ierr)
            if (ierr .ne. 0) then
                write(6,"('Memory error when allocating arrayatdx, ... arrayatdzrank  in gridrep &
...
! 	Grid tabulation
    ...
    do iz =  1, nz
        ...
        do iy = 1, ny
            ...
            do ix =1, nx
                ...
                do ia = 1, ncen
                    ...
                    if (lminrep .eq. 1) then
                        if(    (.not. lboundsx .or. (x .gt. xboundinf .and. x .lt. xboundsup)) .and. &
                            ...
                            qdeform = qdeform + denrep
                            if (latomics .and. lselat(ia)) then
                                if (denrep .gt. cero) then
                                    qdefposrank(isel) = qdefposrank(isel) + denrep
                                else
                                    qdefnegrank(isel) = qdefnegrank(isel) + denrep
                                endif
                            endif
                        endif
                    endif
                enddo
                if (lminrep .eq. 1) then
                    if (qdeform .gt. cero) then
                        qdefposrank(0) = qdefposrank(0) + qdeform
                    else
                        qdefnegrank(0) = qdefnegrank(0) + qdeform
                    endif
                endif
            enddo
        enddo
    enddo
    if (lminrep .eq. 1) then
        call MPI_REDUCE(qdefposrank, qdefpos, SIZE(qdefpos), MPI_REAL8, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
        if (ierr .ne. 0 .and. myrank .eq. 0) then
            write(6,"('Error in MPI_REDUCE for qdefpos,   ierr = ',i7)") ierr
            abort = 1
            return
        endif
        call MPI_REDUCE(qdefnegrank, qdefneg, SIZE(qdefneg), MPI_REAL8, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
        if (ierr .ne. 0 .and. myrank .eq. 0) then
            write(6,"('Error in MPI_REDUCE for qdefpos,   ierr = ',i7)") ierr
            abort = 1
            return
...
!                deallocate(arraydxx, arraydxy, arraydxz, arraydyy, arraydyz, arraydzz)
        ...
        if (lminrep .eq. 1) then
            ...
            if (latomics) then
                ...
                do i = 1, nsel
                    if (lselat(i)) write(6,"(7x, a2, i5, 8x, f12.5, 12x, f12.5)") atmnam(iatomsel(i)), iatomsel(i) &
...
!   ***************************************************************
  ...
  subroutine dzlm2y(lmax, idimzlm, zlma, zlmady, zlmadz)
    ...
    zlmady(1) = cero	! Derivatives of the S spherical harmonics
    zlmadz(1) = cero
    if (lmax .eq. 0) return
    zlmady(2) = zlma(1)     ! Derivatives of the P spherical harmonics
    zlmady(3) = cero
    zlmady(4) = cero
    zlmadz(2) = cero
    zlmadz(3) = zlma(1)
    zlmadz(4) = cero
    if (lmax .eq. 1) return
    zlmady(5) = re(6) * zlma(4)     ! Derivatives of the D spherical harmonics
    zlmady(6) = re(3) * zlma(3)
    zlmady(7) = -zlma(2)
    zlmady(8) = cero
    zlmady(9) = -re(6) * zlma(2)
    ...
    zlmadz(9) = cero
    if (lmax .eq. 2) return
    zlmady(10) = re(15) * zlma(9)   ! Derivatives of the F spherical harmonics
    zlmady(11) = re(10) * zlma(8)
    zlmady(12) = re(6) * zlma(7) + umed * zlma(9)
    zlmady(13) = -zlma(6)
    zlmady(14) = -umed * zlma(5)
